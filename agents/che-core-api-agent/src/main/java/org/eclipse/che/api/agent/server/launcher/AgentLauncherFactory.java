...
/**
 * Provides {@link AgentLauncher} for specific agent to be run on instance. Returning agent depends
 * on machine type. If no agent found then the default one will be returned.
 *
...
public class AgentLauncherFactory {
  ...
  /**
   * Find launcher for given agent independently of version. If the specific {@link AgentLauncher}
   * isn't registered then the default one will be used.
   *
   * @see Agent#getId()
