...
public class AgentSorter {
   ...
   * @throws AgentException if circular dependency found or agent creation failed or other
   *     unexpected error
   */
  ...
  private void doSort(AgentKey agentKey, List<AgentKey> sorted, Set<String> pending)
      throws AgentException {
    String agentId = agentKey.getId();

    Optional<AgentKey> alreadySorted =
        sorted.stream().filter(k -> k.getId().equals(agentId)).findFirst();
    ...
    for (String dependency : agent.getDependencies()) {
      if (pending.contains(dependency)) {
        throw new AgentException(
            String.format(
                "Agents circular dependency found between '%s' and '%s'", dependency, agentId));
      }
