...
public class SystemInfo {
  ...
  private static final SystemResources JDK_SYSTEM_RESOURCES =
      new SystemResources() {
        @Override
        public int cpu() {
          if (CPU_USAGE != null) {
  ...
  private static final SystemResources LINUX_SYSTEM_RESOURCES =
      new SystemResources() {
        /*
        ...
        private int[] readCpuTimes() throws IOException {
          ...
          try (BufferedReader reader =
              Files.newBufferedReader(
                  FileSystems.getDefault().getPath("/proc/stat"), Charset.forName("UTF-8"))) {
        ...
        private long readMem(String kindOf) throws IOException {
          ...
          try (BufferedReader reader =
              Files.newBufferedReader(
                  FileSystems.getDefault().getPath("/proc/meminfo"), Charset.forName("UTF-8"))) {
