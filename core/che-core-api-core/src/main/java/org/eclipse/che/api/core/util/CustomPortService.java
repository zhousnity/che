...
/**
 * Helps to find free ports. Usage:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Note: It is important to release port when it is not needed any more, otherwise it will be not
 * possible to reuse ports.
 *
 * @author andrew00x
 * @see #MIN_PORT
 * @see #MAX_PORT
...
public class CustomPortService {
  ...
  public CustomPortService(Pair<Integer, Integer> range) {
    if (range.first < 0 || range.second > 65535) {
      throw new IllegalArgumentException(
          String.format("Invalid port range: [%d:%d]", range.first, range.second));
    }
    this.range = range;
  ...
  /**
   * This service stores allocated ports in internal storage to avoid checking ports that already in
   * use. After calling this method storage is cleared. For next port allocation this service will
   * iterates through range of configured ports until finds free port. It may be expensive since
   * checking port means trying to open {@link ServerSocket} and {@link DatagramSocket} on each port
   * in the range.
   *
   * @see #MIN_PORT
   * @see #MAX_PORT
  ...
  /**
   * Returns range of ports that service uses for lookup free port. Modifications to the returned
   * {@code Pair} will not affect the internal {@code Pair}.
   */
