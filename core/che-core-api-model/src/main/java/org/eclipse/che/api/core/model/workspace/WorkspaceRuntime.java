...
public interface WorkspaceRuntime {
  ...
  /** Returns a workspace root folder. The base folder for the workspace projects. */
  @Nullable
  String getRootFolder();

  /**
   * Returns development machine only if its status is either {@link MachineStatus#RUNNING running}
   * or {@link MachineStatus#DESTROYING destroying}, otherwise returns null which means that machine
   * is starting or hasn't been started yet.
   *
   * <p>Returned machine used for extensions management. It is guaranteed that configuration of that
   * machine exists in the active environment.
   *
   * <p>There is a contract between this method and {@link #getMachines()} method, if this method
   * returns null then {@code getMachines()} method returns an empty list, if this method returns
   * dev-machine then {@code getMachines()} method result includes dev-machine.
   */
  @Nullable
  Machine getDevMachine();

  /**
   * Returns all the machines which statuses are either {@link MachineStatus#RUNNING running} or
   * {@link MachineStatus#DESTROYING}.
   *
