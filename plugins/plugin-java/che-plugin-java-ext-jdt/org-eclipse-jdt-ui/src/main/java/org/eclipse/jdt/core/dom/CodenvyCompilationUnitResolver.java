...
public class CodenvyCompilationUnitResolver extends CompilationUnitResolver {
   ...
   */
  public CodenvyCompilationUnitResolver(
    ...
    try {
      ...
      if (resolver.hasCompilationAborted) {
        ...
        CompilationUnitDeclaration unitDeclaration =
            CompilationUnitResolver.parse(sourceUnit, null, options, flags);
        if (unit != null) {
  ...
  public static CompilationUnit convert(
    ...
    if (needToResolveBindings) {
      resolver =
          new DefaultBindingResolver(
              ...
              owner,
              bindingTables,
              (flags & ICompilationUnit.ENABLE_BINDINGS_RECOVERY) != 0,
              fromJavaProject);
    ...
    compilationUnit.setLineEndTable(
        compilationUnitDeclaration.compilationResult.getLineSeparatorPositions());
