...
public class ParticipantTesting {
  ...
  public static void init() {
    CheRefactoringParticipantsRegistry.registerParticipant(
        "renameParticipants", TestRenameParticipantShared.class);
    CheRefactoringParticipantsRegistry.registerParticipant(
        "renameParticipants", TestRenameParticipantSingle.class);

    CheRefactoringParticipantsRegistry.registerParticipant(
        "moveParticipants", TestMoveParticipantShared.class);
    CheRefactoringParticipantsRegistry.registerParticipant(
        "moveParticipants", TestMoveParticipantSingle.class);
    CheRefactoringParticipantsRegistry.registerParticipant(
        "createParticipants", TestCreateParticipantSingle.class);
    CheRefactoringParticipantsRegistry.registerParticipant(
        "createParticipants", TestCreateParticipantShared.class);

    CheRefactoringParticipantsRegistry.registerParticipant(
        "deleteParticipants", TestDeleteParticipantShared.class);
    CheRefactoringParticipantsRegistry.registerParticipant(
  ...
  public static String[] createHandles(Object[] elements) {
    ...
    for (int i = 0; i < elements.length; i++) {
      ...
      if (element instanceof IJavaElement) {
        ...
        result.add(
            ((JavaElementResourceMapping) element).getJavaElement().getHandleIdentifier()
                + "_mapping");
      }
    }
  ...
  public static void testSimilarElements(
      List similarList, List similarNewNameList, List similarNewHandleList) {
    ...
    if (similarList.size() == 0) {
      ...
      TestRenameParticipantShared.testSimilarElements(
          similarList, similarNewNameList, similarNewHandleList);
    }
  }
