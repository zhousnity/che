...
/**
 * A copy target edit denotes the target of a copy operation. Copy target edits are only valid
 * inside an edit tree if they have a corresponding source edit. Furthermore a target edit can't
 * can't be a direct or indirect child of the associated source edit. Violating one of two
 * requirements will result in a <code>
 * MalformedTreeException</code> when executing the edit tree.
 *
 * <p>Copy target edits can't be used as a parent for other edits. Trying to add an edit to a copy
