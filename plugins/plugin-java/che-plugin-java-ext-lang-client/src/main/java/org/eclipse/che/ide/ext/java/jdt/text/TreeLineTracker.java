...
abstract class TreeLineTracker implements LineTracker {
  ...
  TreeLineTracker(ListLineTracker tracker) {
    ...
    if (node.delimiter != NO_DELIM) insertAfter(node, 0, NO_DELIM);
  ...
  private int offsetByLine(final int line) throws BadLocationException {
    ...
    while (true) {
      ...
      if (remaining == node.line) return offset + node.offset;
  ...
  private void setChild(Node parent, Node child, boolean isLeftChild) {
    if (parent == null) {
      if (child == null) fRoot = new Node(0, NO_DELIM);
      else fRoot = child;
    } else {
  ...
  private Node insertAfter(Node node, int length, String delimiter) {
    ...
    if (node.right == null) setChild(node, added, false);
    else setChild(successorDown(node.right), added, true);
  ...
  private void updateParentBalanceAfterInsertion(Node node) {
    ...
    while (parent != null) {
      if (node == parent.left) parent.balance--;
      else parent.balance++;
  ...
  private void updateParentBalanceAfterDeletion(Node node, boolean wasLeftChild) {
    while (node != null) {
      if (wasLeftChild) node.balance++;
      else node.balance--;

      Node parent = node.parent;
      if (parent != null) wasLeftChild = node == parent.left;
      ...
      switch (node.balance) {
        ...
        case -2:
          if (rebalanceAfterDeletionRight(node.left)) return;
          break; // propagate up
        case 2:
          if (rebalanceAfterDeletionLeft(node.right)) return;
          break; // propagate up
        case 0:
          break; // propagate up
        default:
  ...
  private Node successor(Node node) {
    if (node.right != null) return successorDown(node.right);
   ...
   * @throws org.eclipse.che.ide.api.editor.text.BadLocationException always
   */
