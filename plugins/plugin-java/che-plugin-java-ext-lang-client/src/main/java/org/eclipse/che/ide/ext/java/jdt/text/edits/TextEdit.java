...
/**
 * A text edit describes an elementary text manipulation operation. Edits are executed by applying
 * them to a document (e.g. an instance of <code>Document
 * </code>).
 *
 * <p>Text edits form a tree. Clients can navigate the tree upwards, from child to parent, as well
 * as downwards. Newly created edits are un-parented. New edits are added to the tree by calling one
 * of the <code>add</code> methods on a parent edit.
 *
 * <p>An edit tree is well formed in the following sense:
 *
 * <ul>
 *   <li>a parent edit covers all its children
 *   <li>children don't overlap
 *   <li>an edit with length 0 can't have any children
 * </ul>
 *
 * Any manipulation of the tree that violates one of the above requirements results in a <code>
 * MalformedTreeException</code>.
 *
 * <p>Insert edits are represented by an edit of length 0. If more than one insert edit exists at
 * the same offset then the edits are executed in the order in which they have been added to a
 * parent. The following code example:
 *
 * <p>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>therefore results in string: "www.eclipse.org".
 *
 * <p>Text edits can be executed in a mode where the edit's region is updated to reflect the edit's
 * position in the changed document. Region updating is enabled by default or can be requested by
 * passing <code>UPDATE_REGIONS</code> to the {@link #apply(Document, int) apply(Document, int)}
 * method. In the above example the region of the <code>InsertEdit(0, "eclipse.")</code> edit after
 * executing the root edit is <code>[3, 8]</code>. If the region of an edit got deleted during
 * change execution the region is set to <code>[-1, -1]</code> and the method {@link #isDeleted()
 * isDeleted} returns <code>true</code>. This class isn't intended to be subclassed outside of the
 * edit framework. Clients are only allowed to subclass <code>MultiTextEdit</code>.
 *
...
public abstract class TextEdit {
  ...
  /**
   * Flags indicating that applying an edit tree to a document is supposed to create a corresponding
   * undo edit. If not specified <code>null</code> is returned from method <code>
   * apply</code>.
   */
  public static final int CREATE_UNDO = 1 << 0;

  /**
   * Flag indicating that the edit's region will be updated to reflect its position in the changed
   * document. If not specified when applying an edit tree to a document the edit's region will be
   * arbitrary. It is even not guaranteed that the tree is still well formed.
   */
  ...
  /**
   * Returns the range that this edit is manipulating. The returned <code>IRegion</code> contains
   * the edit's offset and length at the point in time when this call is made. Any subsequent
   * changes to the edit's offset and length aren't reflected in the returned region object.
   *
   * <p>Creating a region for a deleted edit will result in an assertion failure.
   *
  ...
  /**
   * Returns the offset of the edit. An offset is a 0-based character index. Returns <code>-1</code>
  ...
  /**
   * Returns the exclusive end position of this edit. The exclusive end position denotes the next
   * character of the region manipulated by this edit. The returned value is the result of the
   ...
   *
   * @return <code>true</code> if the edit has been deleted; otherwise <code>false</code> is
   *     returned.
   */
  ...
  /**
   * Move all offsets in the tree by the given delta. This node must be a root node. The resulting
   * offsets must be greater or equal to zero.
   *
  ...
  public boolean covers(TextEdit other) {
    if (getLength() == 0 && !canZeroLengthCover()) return false;

    if (!other.isDefined()) return true;
  ...
  /**
   * Returns <code>true</code> if an edit with length zero can cover another edit. Returns <code>
  ...
  /**
   * Returns the edit's parent. The method returns <code>null</code> if this edit hasn't been add to
   * another edit.
   *
   ...
   * @throws MalformedTreeException is thrown if the child edit can't be added to this edit. This is
   *     the case if the child overlaps with one of its siblings or if the child edit's region isn't
   *     fully covered by this edit.
   */
   ...
   * @throws MalformedTreeException is thrown if one of the given edits can't be added to this edit.
  ...
  /**
   * Removes the edit specified by the given index from the list of children. Returns the child edit
   * that was removed from the list of children. The parent of the returned edit is set to <code>
   * null</code>.
   *
   ...
   * @return <code>true</code> if the edit contained the given child; otherwise <code>false</code>
   *     is returned
   */
  ...
  /**
   * Removes all child edits from and returns them. The parent of the removed edits is set to <code>
   * null</code>.
   *
   * @return an array of the removed edits
   */
  public final TextEdit[] removeChildren() {
    if (fChildren == null) return EMPTY_ARRAY;
  ...
  /**
   * Returns the text range spawned by the given array of text edits. The method requires that the
   * given array contains at least one edit. If all edits passed are deleted the method returns
   * <code>
   * null</code>.
   *
   * @param edits an array of edits
   * @return the text range spawned by the given array of edits or <code>null</code> if all edits
  ...
  public static Region getCoverage(TextEdit[] edits) {
    ...
    if (edits.length == deleted) return null;
   ...
   * @return <code>true</code> iff <code>this == obj</code>; otherwise <code>false</code> is
   *     returned
  ...
  /**
   * The <code>Edit</code> implementation of this <code>Object</code> method calls uses <code>
   * Object#hashCode()</code> to compute its hash code.
   *
  ...
  /**
   * Adds the string representation of this text edit without children information to the given
   * string buffer.
   *
  ...
  /**
   * This method is called on every edit of the copied tree to do some post-processing like
   * connected an edit to a different edit in the tree.
   *
   * <p>This default implementation does nothing
   *
   * @param copier the copier that manages a map between original and copied edit.
   * @see TextEditCopier
   */
  ...
  /**
   * Accepts the given visitor on a type-specific visit of the current edit. This method must be
   * implemented in all concrete text edits.
   *
   * <p>General template for implementation on each concrete TextEdit class:
   *
   * <p>
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Note that the caller (<code>accept</code>) takes care of invoking <code>
   * visitor.preVisit(this)</code> and <code>visitor.postVisit(this)</code>.
   *
   * @param visitor the visitor object
   */
   ...
   *
   * <p>This method must be used by the concrete implementations of <code>accept</code> to traverse
   * list-values properties; it encapsulates the proper handling of on-the-fly changes to the list.
   *
   * @param visitor the visitor object
   */
  protected final void acceptChildren(TextEditVisitor visitor) {
    if (fChildren == null) return;
  ...
  int traverseDocumentUpdating(TextEditProcessor processor, Document document)
    ...
    if (processor.considerEdit(this)) {
      if (delta != 0) adjustLength(delta);
      int r = performDocumentUpdating(document);
      if (r != 0) adjustLength(r);
      delta += r;
    }
    return delta;
  }
  ...
  /**
   * Hook method called when the document updating of a child edit has been completed. When a client
   * calls {@link #apply(Document)} or {@link #apply(Document, int)} this method is called {@link
   * #getChildrenSize()} times.
   *
   * <p>May be overridden by subclasses of {@link MultiTextEdit}.
   *
   * @since 3.1
   */
  ...
  int traverseRegionUpdating(
    ...
    if (fChildren != null) {
      ...
      for (Iterator<TextEdit> iter = fChildren.iterator(); iter.hasNext(); ) {
        ...
        accumulatedDelta =
            child.traverseRegionUpdating(processor, document, accumulatedDelta, childDelete);
        childRegionUpdated();
      }
    }
  ...
  /**
   * Hook method called when the region updating of a child edit has been completed. When a client
   * calls {@link #apply(Document)} this method is called {@link #getChildrenSize()} times. When
   * calling {@link #apply(Document, int)} this method is called {@link #getChildrenSize()} times,
   * when the style parameter contains the {@link #UPDATE_REGIONS} flag.
   *
   * <p>May be overridden by subclasses of {@link MultiTextEdit}.
   *
   * @since 3.1
