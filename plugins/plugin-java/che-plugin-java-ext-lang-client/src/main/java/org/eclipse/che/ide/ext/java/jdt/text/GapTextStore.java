 ...
 *
 * <p><strong>Performance:</strong> Typing-style changes perform in constant time unless
 * re-allocation becomes necessary. Generally, a change that does not cause re-allocation will cause
 * at most one {@linkplain System#arraycopy(Object, int, Object, int, int) arraycopy} operation of a
 * length of about <var>d</var>, where <var>d</var> is the distance from the previous change. Let
 * <var>a(x)</var> be the algorithmic performance of an <code>arraycopy</code> operation of the
 * length <var>x</var>, then such a change then performs in <i>O(a(x))</i>, {@linkplain #get(int,
 * int) get(int, <var>length</var>)} performs in <i>O(a(length))</i>, {@link #get(int)} in
 * <i>O(1)</i>.
 *
 * <p>How frequently the array needs re-allocation is controlled by the constructor parameters.
 *
...
public class GapTextStore implements TextStore {
  ...
  /**
   * Equivalent to {@linkplain GapTextStore#GapTextStore(int, int, float) new GapTextStore(256,
   * 4096, 0.1f)}.
   *
   * @since 3.3
   */
   ...
   * Re-allocation is controlled by the <em>gap factor</em> , which is the quotient of the gap size
   * and the array size. Re-allocation occurs if a change causes the gap factor to go outside <code>
   * [0,&nbsp;maxGapFactor]</code>. When re-allocation occurs, the array is sized such that the gap
   * factor is <code>0.5 * maxGapFactor</code>. The gap size computed in this manner is bounded by
   * the <code>minSize</code> and <code>maxSize</code> parameters.
   *
   * <p>A <code>maxGapFactor</code> of <code>0</code> creates a text store that never has a gap at
   * all (if <code>minSize</code> is 0); a <code>maxGapFactor</code> of <code>1</code> creates a
   * text store that doubles its size with every re-allocation and that never shrinks.
   *
   * <p>The <code>minSize</code> and <code>maxSize</code> parameters are absolute bounds to the
   * allocated gap size. Use <code>minSize</code> to avoid frequent re-allocation for small
   * documents. Use <code>maxSize</code> to avoid a huge gap being allocated for large documents.
   *
   * @param minSize the minimum gap size to allocate (&gt;=&nbsp;0; use 0 for no minimum)
   * @param maxSize the maximum gap size to allocate (&gt;=&nbsp;minSize; use {@link
   *     Integer#MAX_VALUE} for no maximum)
   * @param maxGapFactor is the maximum fraction of the array that is occupied by the gap ( <code>
   *     0&nbsp;&lt;=&nbsp;maxGapFactor&nbsp;&lt;=&nbsp;1</code> )
   * @since 3.3
   */
  ...
  public final char get(int offset) {
    if (offset < fGapStart) return fContent[offset];
  ...
  public final String get(int offset, int length) {
    if (fGapStart <= offset) return new String(fContent, offset + gapSize(), length);

    final int end = offset + length;

    if (end <= fGapStart) return new String(fContent, offset, length);
  ...
  public final void replace(int offset, int length, String text) {
    if (text == null) {
      ...
      if (textLength != 0) text.getChars(0, textLength, fContent, offset);
    }
  }
