...
/**
 * A multi-text edit can be used to aggregate several edits into one edit. The edit itself doesn't
 * modify a document.
 *
 * <p>Clients are allowed to implement subclasses of a multi-text edit.Subclasses must implement
 * <code>doCopy()</code> to ensure the a copy of the right type is created. Not implementing <code>
 * doCopy()</code> in subclasses will result in an assertion failure during copying.
 */
public class MultiTextEdit extends TextEdit {
  ...
  /**
   * Creates a new <code>MultiTextEdit</code>. The range of the edit is determined by the range of
   * its children.
   *
   * <p>Adding this edit to a parent edit sets its range to the range covered by its children. If
   * the edit doesn't have any children its offset is set to the parent's offset and its length is
   * set to 0.
   */
  ...
  /**
   * Creates a new </code>MultiTextEdit</code> for the given range. Adding a child to this edit
   * which isn't covered by the given range will result in an exception.
   *
   ...
   *
   * <p>Note that this method <b>should only be called</b> by the edit framework and not by normal
   * clients.
   *
   * <p>This default implementation does nothing. Subclasses may override if needed.
   *
   * @throws MalformedTreeException if the edit isn't in a valid state and can therefore not be
   *     executed
   */
