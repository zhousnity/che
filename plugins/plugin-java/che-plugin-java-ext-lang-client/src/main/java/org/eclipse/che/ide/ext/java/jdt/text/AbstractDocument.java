...
/**
 * Abstract default implementation of <code>IDocument</code> and its extension interfaces {@link
 * org.eclipse.jface.text.IDocumentExtension}, {@link org.eclipse.jface.text.IDocumentExtension2},
 * {@link org.eclipse.jface.text.IDocumentExtension3}, {@link
 * org.eclipse.jface.text.IDocumentExtension4}, as well as {@link
 * org.eclipse.jface.text.IRepairableDocument}.
 *
 * <p>
 *
 * <p>An <code>AbstractDocument</code> supports the following implementation plug-ins:
 *
 * <ul>
 *   <li>a text store implementing {@link TextStore.text.ITextStore} for storing and managing the
 *       document's content,
 *   <li>a line tracker implementing {@link
 *       org.eclipse.che.ide.legacy.client.api.text.eclipse.LineTracker.text.ILineTracker} to map
 *       character positions to line numbers and vice versa
 * </ul>
 *
 * The document can dynamically change the text store when switching between sequential rewrite mode
 * and normal mode.
 *
 * <p>
 *
 * <p>This class must be subclassed. Subclasses must configure which implementation plug-ins the
 * document instance should use. Subclasses are not intended to overwrite existing methods.
 *
...
public abstract class AbstractDocument implements Document {
   ...
   * must first initialize the implementation plug-ins and then call <code>completeInitialization
   * </code>. Results in the construction of an empty document.
   */
  ...
  /**
   * Returns the document's text store. Assumes that the document has been initialized with a text
   * store.
   *
  ...
  /**
   * Returns the document's line tracker. Assumes that the document has been initialized with a line
   * tracker.
   *
  ...
  /**
   * Initializes document listeners, positions, and position updaters. Must be called inside the
   * constructor after the implementation plug-ins have been set.
   */
  ...
  public void addPosition(String category, Position position)
    ...
    if (category == null) throw new BadPositionCategoryException();

    if ((0 > position.offset)
        || (0 > position.length)
        || (position.offset + position.length > getLength())) throw new BadLocationException();
    ...
    if (endPositions == null) throw new BadPositionCategoryException();
    endPositions.add(
        computeIndexInPositionList(endPositions, position.offset + position.length - 1, false),
        position);
  }
  ...
  protected int computeIndexInPositionList(
      List<Position> positions, int offset, boolean orderedByOffset) {
  ...
  public int computeIndexInCategory(String category, int offset)
    ...
    if (0 > offset || offset > getLength()) throw new BadLocationException();

    List<Position> c = fPositions.get(category);
    if (c == null) throw new BadPositionCategoryException();

    return computeIndexInPositionList(c, offset, true);
  }

  /**
   * Fires the document partitioning changed notification to all registered document partitioning
   * listeners. Uses a robust iterator.
   *
  ...
  protected void fireDocumentPartitioningChanged(DocumentPartitioningChangedEvent event) {
    if (fDocumentPartitioningListeners == null) return;
    ...
    if (fDocumentPartitioners != null) {
      ...
      for (String partitioning : fDocumentPartitioners.keySet()) {
        ...
        if (partitioner.documentChanged(event))
          fDocumentPartitioningChangedEvent.setPartitionChange(
              partitioning, 0, event.getDocument().getLength());
      }
    }

    if (fPositions.size() > 0) updatePositions(event);
  }

  /**
   * Notifies all listeners about the given document change.
   *
   * <p>Executes all registered post notification replace operation.
   *
   * <p>
   *
   * @param event the event to be sent out
  ...
  protected void doFireDocumentChanged(DocumentEvent event) {
    ...
    if (p != null && !p.isEmpty()) fireDocumentPartitioningChanged(p);
  ...
  public char getChar(int pos) throws BadLocationException {
    if ((0 > pos) || (pos >= getLength())) throw new BadLocationException();
  ...
  public String[] getPositionCategories() {
    ...
    for (int i = 0; i < categories.length; i++) categories[i] = keys.next();
    return categories;
  }
  ...
  public String get(int pos, int length) throws BadLocationException {
    ...
    if ((0 > pos) || (0 > length) || (pos + length > myLength)) throw new BadLocationException();
  ...
  public void insertPositionUpdater(PositionUpdater updater, int index) {
    ...
    for (int i = fPositionUpdaters.size() - 1; i >= 0; i--) {
      if (fPositionUpdaters.get(i) == updater) return;
    }

    if (index == fPositionUpdaters.size()) fPositionUpdaters.add(updater);
    else fPositionUpdaters.add(index, updater);
  }
  ...
  public void removePositionCategory(String category) throws BadPositionCategoryException {
    ...
    if (!containsPositionCategory(category)) throw new BadPositionCategoryException();
  ...
  private long getNextModificationStamp() {
    if (fNextModificationStamp == Long.MAX_VALUE
        || fNextModificationStamp == UNKNOWN_MODIFICATION_STAMP) fNextModificationStamp = 0;
    else fNextModificationStamp = fNextModificationStamp + 1;
  ...
  public void replace(int pos, int length, String text, long modificationStamp)
    ...
    if ((0 > pos) || (0 > length) || (pos + length > getLength())) throw new BadLocationException();
  ...
  public void replace(int pos, int length, String text) throws BadLocationException {
    ...
    else replace(pos, length, text, getNextModificationStamp());
  }
  ...
  /**
   * Updates all positions of all categories to the change described by the document event. All
   * registered document updaters are called in the sequence they have been arranged.
   *
  ...
  /**
   * Resumes the document listener notification by sending out the remembered partition changed and
   * document event.
   *
   * @since 2.1
   */
   ...
   * @since 3.0
   */
  public TypedRegion[] computePartitioning(
  ...
  public String getContentType(String partitioning, int offset, boolean preferOpenPartitions)
    ...
    } else if (DEFAULT_PARTITIONING.equals(partitioning)) return DEFAULT_CONTENT_TYPE;
    else throw new BadPartitioningException();
  }
  ...
  public String[] getLegalContentTypes(String partitioning) throws BadPartitioningException {
    DocumentPartitioner partitioner = getDocumentPartitioner(partitioning);
    if (partitioner != null) return partitioner.getLegalContentTypes();
    if (DEFAULT_PARTITIONING.equals(partitioning)) return new String[] {DEFAULT_CONTENT_TYPE};
  ...
  public String[] getPartitionings() {
    if (fDocumentPartitioners == null) return new String[0];
  ...
  /**
   * A list of positions in the given category with an offset inside the given region. The order of
   * the positions is arbitrary.
   *
  ...
  private List<Position> getStartingPositions(String category, int offset, int length)
    ...
    if (positions == null) throw new BadPositionCategoryException();
  ...
  /**
   * A list of positions in the given category with an end position inside the given region. The
   * order of the positions is arbitrary.
   *
   * @param category the position category
   * @param offset the offset of the region
   * @param length the length of the region
