...
public class FindUsagesPresenter extends BasePresenter implements FindUsagesView.ActionDelegate {
  ...
  public void findUsages(TextEditor activeEditor) {
    ...
    if (virtualFile instanceof Resource) {
      ...
      final Optional<Resource> srcFolder =
          ((Resource) virtualFile).getParentWithMarker(SourceFolderMarker.ID);
      ...
      promise
          .then(
              new Operation<FindUsagesResponse>() {
                @Override
              ...
              new Operation<PromiseError>() {
                ...
                public void apply(PromiseError arg) throws OperationException {
                  ...
                  if (cause
                    ...
                    handleError(
                        ((org.eclipse.che.ide.websocket.rest.exceptions.ServerException) cause)
                            .getHTTPStatus(),
                        cause.getMessage());
                    return;
                  }
  ...
  private void handleError(int statusCode, String message) {
    if (statusCode == HTTPStatus.BAD_REQUEST) {
      manager.notify(
          localizationConstant.failedToProcessFindUsage(),
          JSONParser.parseLenient(message).isObject().get("message").isString().stringValue(),
          FAIL,
          FLOAT_MODE);
    } else {
