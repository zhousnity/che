...
public class MachineProviderImplTest {
  ...
  @Mock private JsonRpcEndpointToMachineNameHolder jsonRpcEndpointToMachineNameHolder;
  ...
  @Test
  public void shouldNotPullDockerImageIfAlwaysPullIsFalseAndTheImageExistLocally()
      throws Exception {
    provider = new MachineProviderBuilder().setDoForcePullImage(false).build();
    doReturn(true).when(provider).isDockerImageExistLocally(anyString());
  ...
  public void shouldReTagBuiltImageWithPredictableOnInstanceCreationFromRecipe() throws Exception {
    ...
    TagParams tagParams =
        TagParams.create(
            registry + "/" + repo + ":" + tag, "eclipse-che/" + machine.getContainerName());
    verify(dockerConnector).tag(eq(tagParams));
    ArgumentCaptor<RemoveImageParams> argumentCaptor =
        ArgumentCaptor.forClass(RemoveImageParams.class);
  ...
  public void shouldCreateContainerOnInstanceCreationFromRecipe() throws Exception {
    ...
    assertEquals(
        argumentCaptor.getValue().getContainerConfig().getImage(),
        "eclipse-che/" + machine.getContainerName());
  }

  @Test
  public void shouldPublishAllExposedPortsOnCreateContainerOnInstanceCreationFromRecipe()
      throws Exception {
    // when
    createInstanceFromRecipe();
  ...
  public void shouldStartContainerOnCreateInstanceFromRecipe() throws Exception {
    ...
    ArgumentCaptor<StartContainerParams> argumentCaptor =
        ArgumentCaptor.forClass(StartContainerParams.class);
  ...
  public void shouldBeAbleToCreateContainerWithCpuSet() throws Exception {
    ...
    verify(dockerConnector).createContainer(argumentCaptor.capture());
    assertEquals(
        argumentCaptor.getValue().getContainerConfig().getHostConfig().getCpusetCpus(), "0-3");
  }
  ...
  public void shouldSetDnsResolversOnContainerCreation(String[] dnsResolvers) throws Exception {
    ...
    verify(dockerConnector).createContainer(argumentCaptor.capture());
    assertEqualsNoOrder(
        argumentCaptor.getValue().getContainerConfig().getHostConfig().getDns(), dnsResolvers);
  }
  ...
  public void shouldSetNullDnsResolversOnContainerCreationByDefault() throws Exception {
    ...
    verify(dockerConnector).createContainer(argumentCaptor.capture());
    assertEqualsNoOrder(
        argumentCaptor.getValue().getContainerConfig().getHostConfig().getDns(), null);
  }
  ...
  public void shouldRemoveContainerInCaseFailedStartContainer() throws Exception {
    doThrow(IOException.class)
        ...
        .startContainer(StartContainerParams.create(CONTAINER_ID));
  ...
  @Test(expectedExceptions = ServerException.class)
  public void shouldRemoveContainerInCaseFailedCreateInstanceOnTheDockerMachineFactory()
      throws Exception {
    doThrow(IOException.class)
        .when(dockerMachineFactory)
        .createInstance(any(), any(), any(), any(), any());
  ...
  @Test
  public void shouldCallCreationDockerInstanceWithFactoryOnCreateInstanceFromRecipe()
      throws Exception {
    CheServiceImpl service = createService();
    createInstanceFromRecipe(service);

    verify(dockerMachineFactory)
        .createInstance(
            any(Machine.class),
            eq(CONTAINER_ID),
  ...
  public void shouldBeAbleToSetCorrectSwapSize(
      double swapMultiplier, int memoryMB, long expectedSwapSize) throws Exception {
    // given
    ...
    verify(dockerConnector).createContainer(argumentCaptor.capture());
    assertEquals(
        argumentCaptor.getValue().getContainerConfig().getHostConfig().getMemorySwap(),
        expectedSwapSize);
  }
  ...
  public void shouldExposeCommonAndDevPortsToContainerOnDevInstanceCreationFromRecipe()
    ...
    final Set<ServerConf> commonServers =
        new HashSet<>(
            asList(
                new ServerConfImpl("reference1", "8080", "http", null),
                new ServerConfImpl("reference2", "8081", "ftp", null)));
    expectedExposedPorts.addAll(
        commonServers.stream().map(ServerConf::getPort).collect(Collectors.toList()));

    final Set<ServerConf> devServers =
        new HashSet<>(
            asList(
                new ServerConfImpl("reference3", "8082", "https", null),
                new ServerConfImpl("reference4", "8083", "sftp", null)));
    expectedExposedPorts.addAll(
        devServers.stream().map(ServerConf::getPort).collect(Collectors.toList()));

    provider =
        new MachineProviderBuilder()
            .setDevMachineServers(devServers)
  ...
  public void shouldExposeOnlyCommonPortsToContainerOnNonDevInstanceCreationFromRecipe()
    ...
    provider = new MachineProviderBuilder().setAllMachineServers(commonServers).build();
  ...
  @Test
  public void shouldExposeCommonAndDevPortsToContainerOnDevInstanceCreationFromSnapshot()
      throws Exception {
    List<String> expectedExposedPorts = new ArrayList<>();
    final Set<ServerConf> commonServers =
        new HashSet<>(
  ...
  @Test
  public void
      shouldAddServersConfigsPortsFromMachineConfigToExposedPortsOnDevInstanceCreationFromRecipe()
          throws Exception {
    // given
    final boolean isDev = true;
    CheServiceImpl machine = createService();
  ...
  public void shouldAddBindMountAndRegularVolumesOnInstanceCreationFromRecipe() throws Exception {
    String[] bindMountVolumesFromMachine =
        new String[] {
          "/my/bind/mount1:/from/host1",
    ...
    String[] expectedBindMountVolumes =
        new String[] {
          "/my/bind/mount1:/from/host1",
          "/my/bind/mount2:/from/host2:ro",
          "/my/bind/mount3:/from/host3:ro,Z"
        };
    Map<String, Volume> expectedVolumes =
        Stream.of("/projects", "/something", "/something/else")
            .collect(toMap(Function.identity(), v -> new Volume()));

    provider =
        new MachineProviderBuilder()
            .setDevMachineVolumes(emptySet())
            .setAllMachineVolumes(emptySet())
            .build();

    CheServiceImpl service = createService();
    service.setVolumes(
        Stream.concat(Stream.of(bindMountVolumesFromMachine), Stream.of(volumesFromMachine))
  ...
  public void shouldAddAllVolumesOnDevInstanceCreationFromRecipe() throws Exception {
    ...
    String[] volumesFromMachine = new String[] {"/projects", "/something", "/something/else"};
    String[] allMachinesSystemVolumes =
        new String[] {
          "/some/thing/else:/home/some/thing/else",
          "/other/path:/home/other/path",
          "/home/other/path2"
        };
    String[] devMachinesSystemVolumes =
        new String[] {
          "/etc:/tmp/etc:ro",
  ...
  @Test
  public void shouldAddCommonsSystemVolumesOnlyOnNonDevInstanceCreationFromRecipe()
      throws Exception {
    String[] bindMountVolumesFromMachine =
        new String[] {
          "/my/bind/mount1:/from/host1",
    ...
    Map<String, Volume> expectedVolumes =
        Stream.of("/projects", "/something", "/something/else", "/home/other/path2")
            .collect(toMap(Function.identity(), v -> new Volume()));
  ...
  @Test
  public void shouldAddCommonsSystemVolumesOnlyOnNonDevInstanceCreationFromSnapshot()
      throws Exception {
    String[] bindMountVolumesFromMachine =
        new String[] {
          "/my/bind/mount1:/from/host1",
  ...
  public void shouldAddExtraHostOnDevInstanceCreationFromRecipe() throws Exception {
    ...
    final String[] extraHosts =
        argumentCaptor.getValue().getContainerConfig().getHostConfig().getExtraHosts();
  ...
  @Test
  public void shouldAddCommonAndDevEnvVariablesToContainerOnDevInstanceCreationFromRecipe()
      throws Exception {
  ...
  @Test
  public void shouldAddCommonAndDevEnvVariablesToContainerOnDevInstanceCreationFromSnapshot()
      throws Exception {
    Set<String> commonEnv = new HashSet<>(asList("ENV_VAR1=123", "ENV_VAR2=234"));
    Set<String> devEnv =
        new HashSet<>(asList("DEV_ENV_VAR1=345", "DEV_ENV_VAR2=456", "DEV_ENV_VAR3=567"));
  ...
  @Test
  public void
      shouldNotAddDevEnvToCommonEnvVariablesToContainerOnNonDevInstanceCreationFromSnapshot()
          throws Exception {
    Set<String> commonEnv = new HashSet<>(asList("ENV_VAR1=123", "ENV_VAR2=234"));
    Set<String> devEnv =
        new HashSet<>(asList("DEV_ENV_VAR1=345", "DEV_ENV_VAR2=456", "DEV_ENV_VAR3=567"));
  ...
  @Test
  public void shouldAddEnvVarsFromMachineConfigToContainerOnNonDevInstanceCreationFromSnapshot()
      throws Exception {
    // given
  ...
  @Test
  public void shouldAddEnvVarsFromMachineConfigToContainerOnDevInstanceCreationFromSnapshot()
      throws Exception {
    // given
    Map<String, String> envVarsFromConfig = new HashMap<>();
    envVarsFromConfig.put("ENV_VAR1", "123");
  ...
  @Test
  public void shouldAddEnvVarsFromMachineConfigToContainerOnDevInstanceCreationFromRecipe()
      throws Exception {
    // given
    Map<String, String> envVarsFromConfig = new HashMap<>();
    envVarsFromConfig.put("ENV_VAR1", "123");
  ...
  public void shouldNotChangeEntrypointCmdIfTheyAreNotIdentified(String[] entrypoint, String[] cmd)
      throws Exception {
    // given
    when(imageConfig.getCmd()).thenReturn(cmd);
    when(imageConfig.getEntrypoint()).thenReturn(entrypoint);
    ...
    verify(dockerConnector).createContainer(argumentCaptor.capture());
    assertEqualsNoOrder(
        argumentCaptor.getValue().getContainerConfig().getEntrypoint(), DEFAULT_ENTRYPOINT);
  ...
  private CheServiceImpl createInstanceFromSnapshot(String repo, String tag, String registry)
  ...
  private void createInstanceFromRecipe(CheServiceImpl service, boolean isDev, String workspaceId)
      throws Exception {
    provider.startService(
        USER_NAME,
        workspaceId,
  ...
  private void createInstanceFromSnapshot(boolean isDev, String workspaceId)
      throws ServerException {
    createInstanceFromSnapshot(createService(), isDev, workspaceId);
  }

  private void createInstanceFromSnapshot(CheServiceImpl service, boolean isDev)
      throws ServerException {
