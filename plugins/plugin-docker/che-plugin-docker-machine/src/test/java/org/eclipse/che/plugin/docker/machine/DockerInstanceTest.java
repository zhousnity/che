...
public class DockerInstanceTest {
  ...
  @Mock private LogMessage logMessageMock;
  @Mock private Exec execMock;
  ...
  public void shouldThrowMachineExceptionWhenExecProblemOccurs() throws Exception {
    when(dockerConnectorMock.createExec(any(CreateExecParams.class)))
  ...
  public void shouldThrowFileNotFoundDuringSedCommand() throws Exception {
    when(logMessageMock.getContent())
        .thenReturn("sed: can't read " + FILE_PATH + ": No such file or directory");
  ...
  public void shouldThrowFileNotFoundDuringCatCommand() throws Exception {
    when(logMessageMock.getContent())
        .thenReturn("cat: " + FILE_PATH + ": No such file or directory");
  ...
  public void shouldCreateDockerImageLocally() throws Exception {
    ...
    verify(dockerConnectorMock, times(1))
        .commit(
  ...
  private DockerInstance getDockerInstance(
      Machine machine, String registry, String container, String image, boolean snapshotUseRegistry)
