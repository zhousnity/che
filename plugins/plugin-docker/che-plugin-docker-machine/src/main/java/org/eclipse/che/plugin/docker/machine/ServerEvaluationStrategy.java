...
/**
 * Represents a strategy for resolving Servers associated with workspace containers. Used to extract
 * relevant information from e.g. {@link ContainerInfo} into a map of {@link ServerImpl} objects.
 *
...
public abstract class ServerEvaluationStrategy {
  ...
  /**
   * @return true if <strong>external</strong> addresses need to be exposed against https, false
   *     otherwise
   */
   ...
   * @param internalAddress address passed into {@code getServers}; used as fallback if address
   *     cannot be retrieved from containerInfo.
   * @return a Map of port protocol (e.g. "4401/tcp") to address (e.g. "172.17.0.1:32317")
   */
  protected abstract Map<String, String> getInternalAddressesAndPorts(
      ContainerInfo containerInfo, String internalAddress);
   ...
   */
  protected abstract Map<String, String> getExternalAddressesAndPorts(
      ContainerInfo containerInfo, String internalAddress);
  ...
  public Map<String, ServerImpl> getServers(
    ...
    Map<String, String> internalAddressesAndPorts =
        getInternalAddressesAndPorts(containerInfo, internalHost);
    Map<String, String> externalAddressesAndPorts =
        getExternalAddressesAndPorts(containerInfo, internalHost);
    ...
    for (String portProtocol : portBindings.keySet()) {
      ...
      ServerProperties properties =
          new ServerPropertiesImpl(serverConf.getPath(), internalAddressAndPort, internalUrl);
  ...
  /**
   * Gets the {@link ServerConfImpl} object associated with {@code portProtocol}. The provided
   * labels should have keys matching e.g.
   *
   * <p>{@code che:server:<portProtocol>:[ref|path|protocol]}
   *
   * @param portProtocol the port binding associated with the server
   * @param labels a map holding the relevant values for reference, protocol, and path for the given
   *     {@code portProtocol}
   * @param serverConfMap a map of {@link ServerConfImpl} with {@code portProtocol} as keys.
   * @return {@code ServerConfImpl}, obtained from {@code serverConfMap} if possible, or from {@code
   *     labels} if there is no entry in {@code serverConfMap}.
   */
  protected ServerConfImpl getServerConfImpl(
      String portProtocol, Map<String, String> labels, Map<String, ServerConfImpl> serverConfMap) {
  ...
  /**
   * Transforms address and server ports into map where key is port and optional transport protocol
   * and value is address port of server.
   *
   * <p>Example: When method accepts address my-host.com and ports:
   *
   * <pre>{@code
   * {
   *     "7070" : [
   ...
   * }</pre>
   */
  protected Map<String, String> getExposedPortsToAddressPorts(
      String address, Map<String, List<PortBinding>> ports, boolean useExposedPorts) {
  ...
  protected Map<String, String> getExposedPortsToAddressPorts(
      String address, Map<String, List<PortBinding>> ports) {
    return getExposedPortsToAddressPorts(address, ports, false);
  }

  /**
   * @param protocolForInternalUrl
   * @return https, if {@link #useHttpsForExternalUrls()} method in sub-class returns true and
   *     protocol for internal Url is http
   */
