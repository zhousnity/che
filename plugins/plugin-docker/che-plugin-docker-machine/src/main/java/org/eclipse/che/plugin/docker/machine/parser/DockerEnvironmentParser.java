...
/**
 * Parser for creating {@link CheServicesEnvironmentImpl} from {@link Environment} which contains a
 * single {@link ExtendedMachine}.
 *
...
public abstract class DockerEnvironmentParser implements TypeSpecificEnvironmentParser {
  ...
  /**
   * Returns machine name which is mapped to {@link ExtendedMachine} in the {@code environment}.
   * Note: Valid {@link Environment} based on docker file or docker image recipe type contains only
   * one {@link ExtendedMachine}.
   *
   * @param environment environment to get machine name
   * @throws IllegalArgumentException in case if {@code environment} is multiple machine environment
   */
