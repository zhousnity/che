...
/**
 * This class is used for generation docker container name or parsing important information from
...
public class DockerContainerNameGenerator implements ContainerNameGenerator {
  ...
  private static final String CONTAINER_NAME_REGEX =
      "^" + NODE_HOST_GROUP + WORKSPACE_ID_GROUP + "_" + MACHINE_ID_GROUP + "_.+$";
  private static final Pattern CONTAINER_NAME_PATTERN = Pattern.compile(CONTAINER_NAME_REGEX);

  /**
   * Return generated name for docker container. Method generate name for docker container in
   * format: <br>
   *
   * <p>workspaceId + "_" + machineId + "_" + userName +"_" + machineName <b>Notice: if generated
   * container name contains incorrect symbols for creation docker container, then we skip this
   * symbols</b>
   *
  ...
  /**
   * Parse machine's {@code containerName} to get information about this container (like
   * workspaceId, machineId). Notice: method doesn't parse information about userName or
   * machineName, because we do not give guarantees about the integrity of this data(see more {@link
   * #generateContainerName(String, String, String, String)}) Notice: container name can contains
   * node host (e.g. "/node-host.dev.box/workspacebbx2_machineic3_user321_ws-machine)
   *
