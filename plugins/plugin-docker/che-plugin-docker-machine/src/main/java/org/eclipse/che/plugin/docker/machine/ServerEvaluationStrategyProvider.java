...
/**
 * Provides {@code ServerEvaluationStrategies}. Intended to be injected into {@link
 * DockerInstanceRuntimeInfo} instances to aid in {@link DockerInstanceRuntimeInfo#getServers()}
 *
 * <p>Strategy to be used is selected by property {@code che.docker.server_evaluation_strategy},
 * which should refer to a strategy bound in {@link LocalDockerModule}. If the strategy cannot be
 * found, the default will be used.
 *
...
public class ServerEvaluationStrategyProvider implements Provider<ServerEvaluationStrategy> {
  ...
  @Inject
  public ServerEvaluationStrategyProvider(
      Map<String, ServerEvaluationStrategy> strategies,
  ...
  private void checkStrategy() throws Exception {
    if (strategy == null) {
      throw new IllegalStateException(
          String.format(
              "Property che.docker.server_evaluation_strategy=%s "
                  + "does not match provided strategies",
              chosenStrategy));
    }
  }
