...
/**
 * The Module for Local Docker components Note that LocalDockerNodeFactory requires
 * machine.docker.local.project parameter pointed to directory containing workspace projects tree
 *
 * @author gazarenkov
...
public class LocalDockerModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    bind(org.eclipse.che.plugin.docker.machine.node.WorkspaceFolderPathProvider.class)
        .to(
            org.eclipse.che.plugin.docker.machine.local.node.provider
                .LocalWorkspaceFolderPathProvider.class);
    ...
    devMachineEnvVars
        .addBinding()
        .toProvider(
            org.eclipse.che.plugin.docker.machine.local.provider.DockerApiHostEnvVariableProvider
                .class);
    ...
    networks
        .addBinding()
        .toProvider(org.eclipse.che.plugin.docker.machine.CheInContainerNetworkProvider.class);
    ...
    extraHosts
        .addBinding()
        .toProvider(
            org.eclipse.che.plugin.docker.machine.local.provider.CheDockerExtraHostProvider.class);
  }
}
