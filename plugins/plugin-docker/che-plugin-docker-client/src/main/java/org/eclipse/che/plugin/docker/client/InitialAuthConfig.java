...
/**
 * Collects auth configurations for private docker registries. Credential might be configured in
 * .properties files, see details {@link org.eclipse.che.inject.CheBootstrap}. Credentials
 * configured as (key=value) pairs. Key is string that starts with prefix {@code
 * che.docker.registry.auth.} followed by url and credentials of docker registry server.
 *
 * <pre>{@code
...
public class InitialAuthConfig {
  ...
  @VisibleForTesting protected static final String CONFIG_PREFIX = "che.docker.registry.auth.";

  @VisibleForTesting
  protected static final String CONFIGURATION_PREFIX_PATTERN =
      "che\\.docker\\.registry\\.auth\\..+";

  @VisibleForTesting
  protected static final String VALID_DOCKER_PROPERTY_NAME_EXAMPLE =
      CONFIG_PREFIX + "registry_name.parameter_name";
  ...
  private String getRegistryPrefix(String propertyName) {
    ...
    if (!URL.equals(propertyIdentifier)
        && !USER_NAME.equals(propertyIdentifier)
        && !PASSWORD.equals(propertyIdentifier)) {
    ...
    for (String defaultRegistryAlias : DEFAULT_REGISTRY_SYNONYMS) {
      if (configMap.containsKey(defaultRegistryAlias)) {
        if (isDockerHubConfigured) {
          throw new IllegalArgumentException(
              "Docker hub registry is configured more than one time");
        }
