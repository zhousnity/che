...
public class CopyViewImpl extends Window implements CopyView {
  ...
  @UiField CheckBox sourceCheckBox;

  @UiField CheckBox targetCheckBox;
  ...
  @UiField DeckPanel commentDeckPanel;
  ...
  public void onDestinationUrlCheckBoxActivated(ClickEvent event) {
    new SlideAnimation()
        .showWidget(
            deckPanel.getWidget(targetCheckBox.getValue() ? 0 : 1),
            deckPanel.getWidget(targetCheckBox.getValue() ? 1 : 0));
    delegate.onTargetCheckBoxChanged();

    new SlideAnimation()
        .showWidget(
            commentDeckPanel.getWidget(targetCheckBox.getValue() ? 1 : 0),
  ...
  public void onSourceTextBoxChanged(KeyUpEvent event) {
    ...
    newNameTextBox
        .getElement()
        .setAttribute(PLACEHOLDER, Strings.isNullOrEmpty(projectName) ? "name" : projectName);
  ...
  private class SlideAnimation extends Animation {
    ...
    /**
     * The fixed height of a {@link com.google.gwt.user.client.ui.TabPanel} in pixels. If the {@link
     * com.google.gwt.user.client.ui .TabPanel} does not have a fixed height, this will be set to
     * -1.
     */
