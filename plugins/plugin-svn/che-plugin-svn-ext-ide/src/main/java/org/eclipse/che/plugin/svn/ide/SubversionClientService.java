...
public interface SubversionClientService {
   ...
   * @param depth the update depth (--depth)
   * @param addIgnored whether or not to add ignored files (--no-ignore)
   * @param addParents whether or not to add parent paths (--parents)
   * @param autoProps whether to explicitly use automatic properties (--auto-props)
   * @param noAutoProps whether to explicitly not use automatic properties (--no-auto-props)
   */
  Promise<CLIOutputResponse> add(
      Path project,
      Path[] paths,
      String depth,
      boolean addIgnored,
   ...
   *
   * @param project the project path
   * @param source source item path
   * @param destination destination path
   * @param credentials {@link Credentials} object that contains user name and password for
   *     authentication
   */
  Promise<CLIOutputResponse> copy(
      Path project,
      Path source,
      Path destination,
      String comment,
   ...
   * @param sourceUrl source URL to merge
   */
   ...
   *
   * @param project relative path to the project in local workspace
   * @param target target to operate
   * @param revision revision, use HEAD to specify latest revision
   * @param credentials {@link Credentials} object that contains user name and password for
   *     authentication
   */
  Promise<InfoResponse> info(
      Path project,
      String target,
      String revision,
      boolean children,
   ...
   * @param paths the paths to update
   * @param depth the update depth (--depth)
   * @param ignoreExternals whether or not to ignore externals (--ignore-externals)
   * @param showIgnored whether or not to show ignored paths (--no-ignored)
   * @param showUpdates whether or not to show repository updates (--show-updates)
   * @param showUnversioned whether or not to show unversioned paths (--quiet)
   * @param verbose whether or not to be verbose (--verbose)
   * @param changeLists which changelists to operation on (--changelist)
   */
  Promise<CLIOutputResponse> status(
      Path project,
      Path[] paths,
      String depth,
  ...
  /**
   * Updates the provided paths, or the working copy as a whole, to the latest, or requested,
   * repository version.
   *
   * @param project the project path
   * @param paths the paths to update
   * @param revision the revision (0 indicates HEAD) (--revision)
   * @param depth the update depth (--depth)
   * @param ignoreExternals whether or not to ignore externals (--ignore-externals)
   * @param accept the accept argument (--accept)
   * @param credentials {@link Credentials} object that contains user name and password for
   *     authentication
   */
  Promise<CLIOutputWithRevisionResponse> update(
      Path project,
      Path[] paths,
      String revision,
      String depth,
   ...
   */
  Promise<CLIOutputWithRevisionResponse> doSwitch(
      String location,
      Path project,
      String revision,
      String depth,
  ...
  Promise<CLIOutputResponse> showDiff(
      Path project, Path[] paths, String revision, @Nullable Credentials credentials);
   ...
   * @param force if false, will warn if another user already has a lock on a target, leave this
   *     target unchanged, and continue.<br>
   *     if true, will steal the lock from the previous owner instead
   * @param credentials {@link Credentials} object that contains user name and password for
   *     authentication
   */
  Promise<CLIOutputResponse> lock(
      Path project, Path[] paths, boolean force, @Nullable Credentials credentials);
   ...
   *     authentication
   */
  Promise<CLIOutputResponse> unlock(
      Path project, Path[] paths, boolean force, @Nullable Credentials credentials);
   ...
   *
   * @param project the project path
   * @param paths the paths to include in the commit
   * @param message the commit message
   * @param keepChangeLists if true, doesn't remove the changelist assigments from working copy
   *     items after committing
   * @param keepLocks if true, doesn't unlock files after commiting
   */
  Promise<CLIOutputWithRevisionResponse> commit(
  ...
  Promise<CLIOutputResponseList> resolve(
      Path project, Map<String, String> resolutions, String depth);
   ...
   *     authentication
   */
  Promise<CLIOutputResponse> move(
      Path project,
      Path source,
      Path destination,
      String comment,
   ...
   * @param propertyValues the property value
   * @param depth the depth
   * @param force forcing or not
   * @param path path to which property sets
   */
  Promise<CLIOutputResponse> propertySet(
      Path project,
      String propertyName,
   ...
   * @param depth the depth
   * @param force forcing or not
   * @param path path from which property should be deleted
   */
  Promise<CLIOutputResponse> propertyDelete(
      Path project, String propertyName, Depth depth, boolean force, Path path);

  /**
   * Get the list of all revisions where a given path was modified
   *
   * @param project the project path
   * @param path path to get the revisions for
   * @param revisionRange the range of revisions to check
   */
   ...
   *
   * @param project the project path
   * @param target the target path to browse
   */
