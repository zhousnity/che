...
public class SubversionProjectImporterPresenter extends AbstractWizardPage<MutableProjectConfig>
  ...
  @Inject
  public SubversionProjectImporterPresenter(
      SubversionExtensionLocalizationConstants constants, SubversionProjectImporterView view) {
    ...
    if (isSubversionUrlCorrect(calcUrl)) {
      ...
      if (isNullOrEmpty(view.getProjectName())) {
        final String projectName =
            Iterables.getLast(Splitter.on("/").omitEmptyStrings().split(projectUrl));
  ...
  private String getUrl(String url, String relPath) {
    return (url.endsWith("/") ? url.substring(0, url.length() - 1) : url)
        + (relPath.startsWith("/") ? relPath : relPath.concat("/"));
  }
}
