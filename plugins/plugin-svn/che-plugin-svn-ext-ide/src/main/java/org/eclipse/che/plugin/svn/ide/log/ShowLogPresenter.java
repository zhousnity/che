...
public class ShowLogPresenter extends SubversionActionPresenter {
  ...
  private void showLogs(String range) {
    ...
    service
        .showLog(project.getLocation(), toRelative(project, resources), range)
        .then(
            new Operation<CLIOutputResponse>() {
              @Override
              public void apply(CLIOutputResponse response) throws OperationException {
                printResponse(
                    response.getCommand(), response.getOutput(), null, constants.commandLog());
              }
            })
        .catchError(
            new Operation<PromiseError>() {
