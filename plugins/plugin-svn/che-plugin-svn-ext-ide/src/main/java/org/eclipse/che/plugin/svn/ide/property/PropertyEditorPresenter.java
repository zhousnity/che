 ...
 */
@Singleton
public class PropertyEditorPresenter extends SubversionActionPresenter
    implements PropertyEditorView.ActionDelegate {
  ...
  @Inject
  protected PropertyEditorPresenter(
  ...
  private void editProperty(Project project) {
    ...
    final StatusNotification notification =
        new StatusNotification(constants.propertyModifyStart(), PROGRESS, FLOAT_MODE);
  ...
  private void deleteProperty(Project project) {
    ...
    final StatusNotification notification =
        new StatusNotification(constants.propertyRemoveStart(), PROGRESS, FLOAT_MODE);
    notificationManager.notify(notification);

    service
        .propertyDelete(
            project.getLocation(), propertyName, depth, force, toRelative(project, resources[0]))
        .then(
            new Operation<CLIOutputResponse>() {
              @Override
              public void apply(CLIOutputResponse response) throws OperationException {
