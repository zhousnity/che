...
public class MergePresenter extends SubversionActionPresenter implements MergeView.ActionDelegate {
        ...
        .then(
            new Operation<InfoResponse>() {
              ...
              public void apply(InfoResponse response) throws OperationException {
                ...
                service
                    .info(project.getLocation(), repositoryRoot, "HEAD", true)
                    .then(
                        new Operation<InfoResponse>() {
                          @Override
                          public void apply(InfoResponse response) throws OperationException {
                            ...
                            SubversionItemNode subversionTreeNode =
                                new SubversionItemNode(response.getItems().get(0));
  ...
  class SubversionItemNode extends AbstractTreeNode implements HasDataObject<SubversionItem> {
    ...
    protected Promise<List<Node>> getChildrenImpl() {
      ...
      return service
          .info(project.getLocation(), getData().getURL(), "HEAD", true)
          .then(
              new Function<InfoResponse, List<Node>>() {
                @Override
