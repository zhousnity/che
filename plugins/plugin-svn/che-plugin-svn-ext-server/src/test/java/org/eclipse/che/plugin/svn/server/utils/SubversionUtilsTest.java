...
public class SubversionUtilsTest {
  ...
  public void testGetCheckoutRevision() throws Exception {
    assertEquals(
        -1, SubversionUtils.getCheckoutRevision(ImmutableList.of("Some output", "Revision ABC")));

    assertEquals(
        -1,
        SubversionUtils.getCheckoutRevision(
            ImmutableList.of("Some output", "Checked out revision ABC")));

    assertEquals(
        1588353,
        SubversionUtils.getCheckoutRevision(
            ImmutableList.of("Some output", "Checked out revision 1588353.")));
  }
  ...
  public void testGetUpdateRevision() throws Exception {
    assertEquals(
        -1,
        SubversionUtils.getUpdateRevision(
            ImmutableList.of("Some output", "Updated to revision ABC")));

    assertEquals(
        -1, SubversionUtils.getUpdateRevision(ImmutableList.of("Some output", "At revision ABC")));

    assertEquals(
        1588353,
        SubversionUtils.getUpdateRevision(
            ImmutableList.of("Some output", "Updated to revision 1588353.")));

    assertEquals(
        1588353,
        SubversionUtils.getUpdateRevision(ImmutableList.of("Some output", "At revision 1588353.")));
  }

  @Test()
  public void testRecognizeProjectUri() throws Exception {
    assertEquals(
        "http://a/b/c/project", recognizeProjectUri("http://a/b/c", "^/project/branches/3.1"));
