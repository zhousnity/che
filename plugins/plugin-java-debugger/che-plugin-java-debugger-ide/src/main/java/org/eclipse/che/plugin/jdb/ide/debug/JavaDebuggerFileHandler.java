...
public class JavaDebuggerFileHandler implements ActiveFileHandler {
  ...
  public void openFile(Location location, AsyncCallback<VirtualFile> callback) {
    ...
    if (activePath != null
        && !activePath.equals(location.getTarget())
        && !activePath.equals(location.getResourcePath())) {
  ...
  private void doOpenFile(final Location location, final AsyncCallback<VirtualFile> callback) {
    appContext
        .getWorkspaceRoot()
        .getFile(location.getResourcePath())
        .then(
        ...
        .then(
            new Operation<JarEntry>() {
              ...
              public void apply(final JarEntry jarEntry) throws OperationException {
                ...
                AsyncCallback<VirtualFile> downloadSourceCallback =
  ...
  private void handleContentGeneratedResource(
      final VirtualFile file, final Location location, final AsyncCallback<VirtualFile> callback) {
    if (handler != null) {
    ...
    handler =
        eventBus.addHandler(
            ...
            new EditorDirtyStateChangedHandler() {
              @Override
  ...
  public void handleActivatedFile(
