...
public class BasicActiveFileHandler implements ActiveFileHandler {
  ...
  /**
   * Tries to open file in the editor. To perform the operation the following sequence of methods
   * invocation are processed:
   *
   * <p>{@link BasicActiveFileHandler#tryFindFileInProject(Location, AsyncCallback)} {@link
   * BasicActiveFileHandler#tryFindFileInWorkspace(Location, AsyncCallback)} {@link
   * BasicActiveFileHandler#trySearchSource(Location, AsyncCallback)}
   *
  ...
  protected void tryFindFileInProject(
  ...
  protected void tryFindFileInWorkspace(
      final Location location, final AsyncCallback<VirtualFile> callback) {
    try {
      appContext
          .getWorkspaceRoot()
  ...
  protected void trySearchSource(
      final Location location, final AsyncCallback<VirtualFile> callback) {
    appContext
        .getWorkspaceRoot()
        ...
        .then(
            new Operation<Resource[]>() {
              ...
              public void apply(Resource[] resources) throws OperationException {
                ...
                appContext
                    .getWorkspaceRoot()
                    .getFile(resources[0].getLocation())
                    .then(
                        new Operation<Optional<File>>() {
                          @Override
                          public void apply(Optional<File> file) throws OperationException {
