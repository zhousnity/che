...
public class TraefikCreateContainerInterceptorTest {
  ...
  @Mock private ImageConfig imageInfoConfig;
  ...
  @InjectMocks private TraefikCreateContainerInterceptor traefikCreateContainerInterceptor;
  ...
  protected void setup() throws Exception {
    ...
    envContainerConfig =
        new String[] {
          "CHE_WORKSPACE_ID=work123", "CHE_MACHINE_NAME=abcd", "CHE_IS_DEV_MACHINE=true"
        };
  ...
  public void testRules() throws Throwable {
    ...
    Assert.assertEquals(
        containerLabels.get("traefik.service-wsagent.frontend.entryPoints"), "http");

    Assert.assertTrue(containerLabels.containsKey("traefik.service-wsagent.frontend.rule"));
    Assert.assertEquals(
        containerLabels.get("traefik.service-wsagent.frontend.rule"),
        "Host:wsagent.abcd.work123.127.0.0.1.nip.io");

    Assert.assertTrue(containerLabels.containsKey("traefik.service-tomcat8.frontend.rule"));
    Assert.assertEquals(
        containerLabels.get("traefik.service-tomcat8.frontend.rule"),
        "Host:tomcat8.abcd.work123.127.0.0.1.nip.io");
  }
