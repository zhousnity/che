/**
 * ***************************************************************************** Copyright (c) 2017
 * RedHat, Inc. All rights reserved. This program and the accompanying materials are made available
 * under the terms of the Eclipse Public License v1.0 which accompanies this distribution, and is
 * available at http://www.eclipse.org/legal/epl-v10.html
 *
 * <p>Contributors: RedHat, Inc. - initial commit
 * *****************************************************************************
 */
...
public class TestServiceClientTest implements MockitoPrinter {
  ...
  @Mock private ExecAgentCommandManager execAgentCommandManager;
  @Mock private PromiseProvider promiseProvider;
  @Mock private MacroProcessor macroProcessor;
  ...
  @Mock private TestGoal testGoal;
  ...
  @Spy private final List<DtoWithPid> consoleEvents = new ArrayList<>();
  ...
  public void initMocks() {
    ...
    when(commandConsoleFactory.create(any(CommandImpl.class), any(Machine.class)))
        .then(
            createCall -> {
  ...
  public void noBuildCommand() {
    when(commandManager.getCommands())
        .thenReturn(
            asList(
                new CommandImpl(
                    "customBuild", "mvn clean install -f ${current.project.path}", "mvn")));
  ...
  public void buildCommandNotAMavenCommand() {
    when(commandManager.getCommands())
        .thenReturn(
            asList(
                new CommandImpl(
                    "build", "mvn clean install -f ${current.project.path}", "someOtherType")));
  ...
  public void sucessfulTestsAfterCompilation() {
    ...
    Promise<TestResult> resultPromise =
        testServiceClient.runTestsAfterCompilation(
            projectPath, testFramework, parameters, statusNotification, compileCommandPromise);
    ...
    verify(statusNotification)
        .setContent("Compiling the project before starting the test session.");
    verify(execAgentCommandManager)
        .startProcess(
            "DevMachineId",
            new CommandImpl(
                "test-compile",
    ...
    Assert.assertEquals(
        eventStrings,
        Arrays.asList("Started", "StdErr - A small warning", "StdOut - BUILD SUCCESS", "Died"));
  }
}
