...
public class SshMachineExecAgentLauncher {
  ...
  private static final Pattern UNAME_OUTPUT =
      Pattern.compile("\\[STDOUT\\] (?<os>[\\S]+) (?<architecture>[\\S]+)");
  private static final String DEFAULT_ARCHITECTURE = "linux_amd64";
  private static final ExecutorService executor =
      Executors.newCachedThreadPool(
          new ThreadFactoryBuilder()
              .setNameFormat("SshAgentLauncher-%d")
  ...
  @Inject
  public SshMachineExecAgentLauncher(
  ...
  public void launch(SshMachineInstance machine, Agent agent) throws ServerException {
    ...
    try {
      ...
      SshProcessLaunchedChecker agentLaunchingChecker =
          new SshProcessLaunchedChecker("che-websocket-terminal");
      ...
      final String errMsg =
          format(
              "Fail launching agent %s. Workspace ID:%s",
  ...
  protected SshMachineProcess start(SshMachineInstance machine, Agent agent)
