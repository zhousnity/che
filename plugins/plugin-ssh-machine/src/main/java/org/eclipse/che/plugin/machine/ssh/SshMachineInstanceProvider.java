...
public class SshMachineInstanceProvider {
  ...
  @Inject
  public SshMachineInstanceProvider(
      SshMachineFactory sshMachineFactory, RecipeDownloader recipeDownloader) throws IOException {
   ...
   * @param lineConsumer output for instance creation logs
   ...
   */
  public SshMachineInstance createInstance(Machine machine, LineConsumer lineConsumer)
    ...
    requireNonNull(
        machine.getConfig().getSource().getLocation(), "Location in machine source is required");
    ...
    SshClient sshClient =
        sshMachineFactory.createSshClient(sshMachineRecipe, machine.getConfig().getEnvVariables());
    sshClient.start();

    SshMachineInstance instance =
        sshMachineFactory.createInstance(machine, sshClient, lineConsumer);
