...
public class JUnitTestRunner implements TestRunner {
  ...
  public JUnitTestRunner(
      Provider<ProjectManager> projectManager, TestClasspathRegistry classpathRegistry) {
  ...
  public TestResult execute(Map<String, String> testParameters) throws Exception {
    ...
    TestClasspathProvider classpathProvider =
        classpathRegistry.getTestClasspathProvider(projectType);
    URLClassLoader providedClassLoader =
        (URLClassLoader)
            classpathProvider.getClassLoader(projectAbsolutePath, projectPath, updateClasspath);
    projectClassLoader =
        new URLClassLoader(providedClassLoader.getURLs(), null) {
          @Override
    ...
    if (!isJUnit4Compatible) {
      if (!isJUnit3Compatible) {
        throw new ClassNotFoundException(
            "JUnit classes not found in the following project classpath: "
  ...
  private TestResult runAll4x(String projectAbsolutePath) throws Exception {
    ...
    Files.walk(Paths.get(projectAbsolutePath, "target", "test-classes"))
        .forEach(
            filePath -> {
              if (Files.isRegularFile(filePath)
                  && filePath.toString().toLowerCase().endsWith(".class")) {
                String path =
                    Paths.get(projectAbsolutePath, "target", "test-classes")
                        .relativize(filePath)
                        .toString();
  ...
  private Object create4xTestListener(
      ClassLoader loader,
      Class<?> listenerClass,
    ...
    MethodHandler mi =
        new MethodHandler() {
          @Override
          public Object invoke(Object self, Method m, Method method, Object[] args)
              throws Throwable {
            String methodName = m.getName();
            ...
            switch (methodName) {
              ...
              case "testAssumptionFailure":
                description =
                    args[0].getClass().getMethod("getDescription", new Class<?>[0]).invoke(args[0]);
                throwable =
                    (Throwable)
                        args[0]
                            .getClass()
                            .getMethod("getException", new Class<?>[0])
                            .invoke(args[0]);
                break;
              default:
                return null;
            ...
            String testKey =
                (String)
                    description
                        .getClass()
                        .getMethod("getDisplayName", new Class<?>[0])
                        .invoke(description);
