...
public interface RecipeServiceClient {
   ...
   * Create recipe.
   *
   * @param newRecipe describer of the recipe
   ...
   * Get all recipes.
   *
   * @return a promise that will provide a list of {@link RecipeDescriptor}s, or rejects with an
   *     error
   */
  Promise<List<RecipeDescriptor>> getAllRecipes();

  /**
   * Search for recipes which type is equal to the specified {@code type} and tags contain all of
   * the specified {@code tags}.
   *
   ...
   *     error
   */
  Promise<List<RecipeDescriptor>> searchRecipes(
      @NotNull List<String> tags, @Nullable String type, int skipCount, int maxItems);

  /**
   * Update recipe.
   *
   * @param recipeUpdate describer of the recipe updater
   * @return a promise that resolves to the {@link RecipeDescriptor}, or rejects with an error
   */
