...
public interface PartPresenter extends Presenter {
  ...
  /**
   * The property id for <code>getTitle</code>, <code>getTitleImage</code> and <code>getTitleToolTip
   * </code>.
   */
  ...
  /**
   * Returns the title SVG image resource of this part. If this value changes the part must fire a
  ...
  /**
   * This method is called when part is going to be closed. Part itself can deny blocking, by
   * calling onFailure() on callback, i.e. when document is being edited and accidentally close
   * button pressed.
   *
   * @param callback
   */
  ...
  /**
   * Adds a listener for changes to properties of this part. Has no effect if an identical listener
  ...
  /**
   * Removes the given property listener from this part. Has no effect if an identical listener is
   * not registered.
   *
   * @param listener a property listener
   */
