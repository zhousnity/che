...
/**
 * An action which has a selected state, and which toggles its selected state when performed. Can be
 * used to represent a menu item with a checkbox, or a toolbar button which keeps its pressed state.
 *
...
public abstract class ToggleAction extends Action implements Toggleable {
  ...
  public final void actionPerformed(final ActionEvent e) {
    ...
    e.getPresentation()
        .firePropertyChange(
            SELECTED_PROPERTY, Boolean.valueOf(!selected), Boolean.valueOf(selected));
  }

  /**
   * Returns the selected (checked, pressed) state of the action.
   *
   * @param e the action event representing the place and context in which the selected state is
   *     queried.
