 ...
 * websocket connection (close, open, error, etc.), checks http/websocket connection to control it's
 * state. Currently there are only two states that a workspace agent can be at: {@link
 * WsAgentState#STARTED} or {@link WsAgentState#STOPPED}.
 *
 * @author Roman Nikitenko
 * @author Valeriy Svydenko
 */
@Singleton
public class WsAgentStateController
  ...
  private class StopCallback implements ConfirmCallback {
    ...
    public void accepted() {
      workspaceServiceClient
          .stop(devMachine.getWorkspaceId(), createSnapshot)
          .then(
              ignored -> {
                if (reloadPage) {
  ...
  private void checkWsAgentHealth() {
    workspaceServiceClient
        .getWsAgentState(devMachine.getWorkspace())
        .then(
            agentHealthState -> {
