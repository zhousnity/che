...
public class DocumentPositionMapImpl implements DocumentPositionMap {
  ...
  protected int computeIndexInPositionList(
      final List<TypedPosition> positions, final int offset, final boolean orderedByOffset) {
    if (positions.size() == 0) {
      return 0;
    }
  ...
  @Override
  public void removePosition(String category, TypedPosition position)
  ...
  private void removeFromPositionsList(
      List<TypedPosition> positions, TypedPosition position, boolean orderedByOffset) {
   ...
   */
  @Override
  public void addPosition(String category, TypedPosition position)
      throws BadLocationException, BadPositionCategoryException {

    if ((0 > position.offset)
        || (0 > position.length)
        || (position.offset + position.length > this.contentLength)) {
      throw new BadLocationException();
    }
  ...
  public void addPosition(final TypedPosition position) throws BadLocationException {
    try {
      ...
      Log.warn(
          DocumentPositionMapImpl.class,
          "Should not happen: DEFAULT_CATEGORY is not a valid category!");
    }
  }

  @Override
  public List<TypedPosition> getPositions(
