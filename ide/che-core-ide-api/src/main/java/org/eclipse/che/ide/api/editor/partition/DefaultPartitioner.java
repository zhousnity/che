...
public class DefaultPartitioner implements DocumentPartitioner {
  ...
  public DefaultPartitioner(
      final PartitionScanner scanner,
  ...
  public void initialize() {
    ...
    this.documentPositionMap.setContentLength(
        this.documentHandle.getDocument().getContentsCharCount());
  }
    ...
    try {
      ...
      while (!token.isEOF()) {
        ...
        if (isSupportedContentType(contentType)) {
          final TypedPosition position =
              new TypedPosition(scanner.getTokenOffset(), scanner.getTokenLength(), contentType);
          current = position;
      ...
      Log.error(
          ...
          "Invalid position: " + String.valueOf(current) + " (max:" + getContentLength() + ").",
          x);
  ...
  private List<TypedRegion> computePartitioning(
      final int offset, final int length, final boolean includeZeroLengthPartitions) {
    ...
    try {
      ...
      Logger.getLogger(DefaultPartitioner.class.getName())
          .fine("Bad position in computePartitioning.");
  ...
  public TypedRegion getPartition(final int offset) {
    ...
    try {
      ...
      Log.warn(
          DefaultPartitioner.class, "Invalid location " + offset + " (max=" + contentLength + ").");
      return defaultRegion();
    } catch (final BadPositionCategoryException e) {
      Log.warn(
          DefaultPartitioner.class,
          "Invalid position cateory... with default category " + positionCategory + "!",
          e);
      return defaultRegion();
    }
    if (index == null) {
   ...
   */
  private static int getFirstIndexEndingAfterOffset(
      final List<TypedPosition> positions, final int offset) {
    int i = -1;
  ...
  protected TypedPosition findClosestPosition(int offset) {
    ...
    try {
      ...
      Log.warn(
          DefaultPartitioner.class,
          "Bad location: " + offset + "(max:" + getContentLength() + ").");
      return null;
    } catch (final BadPositionCategoryException e) {
