...
/**
 * This interface defines the model for managing annotations attached to a document. The model
 * maintains a set of annotations for a given document and notifies registered annotation model
 * listeners about annotation model changes. It also provides methods for querying the current
 * position of an annotation managed by this model.<br>
 * The following events are produced:
 *
 * <ul>
 *   <li>{@link AnnotationModelEvent} when the {@link AnnotationModel} changes (the changes are
 *       batched)
 *   <li>{@link ClearAnnotationModelEvent} when he {@link AnnotationModel} is cleared
 * </ul>
 *
 * The model handles the following events:
 *
 * <ul>
 *   <li>{@link QueryAnnotationsEvent} where a component requests a list of annotations in a given
 *       range and the model answers using the callback.
 * </ul>
 */
public interface AnnotationModel
    extends UseDocumentHandle, DocumentChangedHandler, QueryAnnotationsHandler {

  /**
   * Adds a annotation to this annotation model. The annotation is associated with with the given
   * position which describes the range covered by the annotation. All registered annotation model
   * listeners are informed about the change. If the model is connected to a document, the position
   * is automatically updated on document changes. If the annotation is already managed by this
   * annotation model or is not a valid position in the connected document nothing happens.
   *
   * @param annotation the annotation to add, may not be <code>null</code>
   * @param position the position describing the range covered by this annotation, may not be <code>
   *     null</code>
   */
  void addAnnotation(Annotation annotation, Position position);

  /**
   * Removes the given annotation from the model. I.e. the annotation is no longer managed by this
   * model. The position associated with the annotation is no longer updated on document changes. If
   * the annotation is not managed by this model, nothing happens.
   *
   ...
   * @return the position of the given annotation or <code>null</code> if no associated annotation
   *     exists
   */
