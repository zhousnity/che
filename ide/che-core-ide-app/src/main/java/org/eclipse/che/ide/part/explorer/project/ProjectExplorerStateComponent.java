    ...
    */
    ...
    for (Node node : projectExplorer.getTree().getNodeStorage().getAll()) {
      if (projectExplorer.getTree().isExpanded(node) && node instanceof ResourceNode) {
        ...
        final List<Node> childrenToStore =
            projectExplorer.getTree().getNodeStorage().getChildren(node);
  ...
  public void loadState(@NotNull JsonObject state) {
    ...
    for (int i = 0; i < paths.length(); i++) {
      ...
      if (revealPromise == null) {
        revealPromise =
            revealer
                .reveal(Path.valueOf(path), false)
                .thenPromise(
                    new Function<Node, Promise<Node>>() {
                      @Override
      ...
      revealPromise
          ...
          .catchError(
              new Function<PromiseError, Node>() {
                @Override
