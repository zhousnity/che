...
public abstract class WorkspaceComponent
    implements Component, WsAgentStateHandler, WorkspaceStoppedEvent.Handler {
  private static final String WS_STATUS_ERROR_MSG =
      "Tried to subscribe to workspace status events, but got error";
  private static final String WS_AGENT_OUTPUT_ERROR_MSG =
      "Tried to subscribe to workspace agent output, but got error";
  private static final String ENV_STATUS_ERROR_MSG =
   ...
   *
   * @param workspace workspace which will be current
   */
   ...
   *
   * @param workspace workspace to listen
   * @param callback callback
   * @param restoreFromSnapshot restore or not the workspace from snapshot
   */
  public void handleWorkspaceEvents(
      final WorkspaceDto workspace,
      final Callback<Component, Exception> callback,
    ...
    if (appContext.getActiveRuntime() != null) {
      appContext
          .getActiveRuntime()
          .getMachines()
          .forEach(machine -> subscribeEnvironmentOutput(machine.getDisplayName()));
    }
                ...
                (Function<Map<String, String>, Map<String, String>>)
                    settings -> {
                      if (Boolean.parseBoolean(
                          settings.getOrDefault(CHE_WORKSPACE_AUTO_START, "true"))) {
                        final WorkspaceConfig config = workspace.getConfig();
                        config
                            .getEnvironments()
                            .get(config.getDefaultEnv())
                            .getMachines()
                            .keySet()
                            .forEach(machine -> subscribeEnvironmentOutput(machine));
                        startWorkspaceById(
                            workspaceId, config.getDefaultEnv(), restoreFromSnapshot);
                      } else {
  ...
  private void subscribeEnvironmentOutput(String machine) {
    ...
    String workspaceIdPlusMachineName = appContext.getWorkspaceId() + "::" + machine;
    ...
    workspaceServiceClient
        ...
        .catchError(
            (Operation<PromiseError>)
                error -> Log.error(getClass(), it + ": " + error.getMessage()));
  }
   ...
   *
   * @param workspace workspace which will be started
   * @param callback callback to be executed
   */
  public void startWorkspace(
      final Workspace workspace, final Callback<Component, Exception> callback) {
  ...
  private void startWorkspaceById(
      String workspaceId, String defaultEnvironment, Boolean restoreFromSnapshot) {
    loader.show(STARTING_WORKSPACE_RUNTIME);
    workspaceServiceClient
        .startById(workspaceId, defaultEnvironment, restoreFromSnapshot)
        .catchError(
