 ...
 */
@Singleton
public class ResolvingProjectStateHolderRegistryImpl
    implements ResolvingProjectStateHolderRegistry {
  private final Map<String, ResolvingProjectStateHolder> resolvingProjectStateHolders =
      new HashMap<>();
  ...
  private void register(Set<ResolvingProjectStateHolder> holders) {
    for (ResolvingProjectStateHolder holder : holders) {
      ...
      if (this.resolvingProjectStateHolders.containsKey(projectType)) {
        Log.warn(
            this.getClass(),
            "Resolving project state holder for '"
                + projectType
                + "' project type is already registered.");
      } else {
