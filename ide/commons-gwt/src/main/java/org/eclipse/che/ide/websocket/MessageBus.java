...
/**
 * WebSocket message bus, that provides two asynchronous messaging patterns: RPC and list-based
 * PubSub.
 *
...
public interface MessageBus extends MessageReceivedHandler {
  ...
  public static enum ReadyState {
    ...
    /**
     * Connection is established and communication is possible. A WebSocket must be in the open
     * state in order to send and receive data over the network.
     */
    OPEN(1),
  ...
  /**
   * Subscribes a new {@link MessageHandler} which will listener for messages sent to the specified
   * channel. Upon the first subscribe to a channel, a message is sent to the server to subscribe
   * the client for that channel. Subsequent subscribes for a channel already previously subscribed
   * to do not trigger a send of another message to the server because the client has already a
   * subscription, and merely registers (client side) the additional handler to be fired for events
   * received on the respective channel.
   *
  ...
  /**
   * Unsubscribes a previously subscribed handler listening on the specified channel. If it's the
   * last unsubscribe to a channel, a message is sent to the server to unsubscribe the client for
   * that channel.
   *
   * @param channel channel identifier
  ...
  /**
   * Unsubscribes a previously subscribed handler listening on the specified channel ignoring
   * errors. If it's the last unsubscribe to a channel, a message is sent to the server to
