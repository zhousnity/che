 ...
 *
 * @param <T> the return type of the response the callback expects. Use {@link Void} for methods
 *     returning {@code void}.
...
public abstract class AsyncRequestCallback<T> implements RequestCallback {
   ...
   * comes callback calls {@link Unmarshallable#unmarshal(com.google.gwt.http.client.Response)}
   ...
   *
   * @param result the response returned from the request. Will be {@code null} if {@code
   *     unmarshaller} wasn't set
   */
