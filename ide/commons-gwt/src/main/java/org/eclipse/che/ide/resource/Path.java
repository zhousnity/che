 ...
 *
 * <p>A path is an ordered collection of string segments, separated by a standard separator
 * character, "/". A path may also have a leading and/or a trailing separator.
 *
 * <p>Note that paths are value objects; all operations on paths return a new path; the path that is
...
public final class Path {
   ...
   * except at the beginning. (to handle UNC paths). All forward slashes ('/') are treated as
   * segment delimiters, and any segment and device delimiters for the local file system are also
   * respected (such as colon (':') and backslash ('\') on some file systems).
   *
   ...
   * handle UNC paths). All forward slashes ('/') are treated as segment delimiters, and any segment
   * delimiters for the local file system are also respected (such as backslash ('\') on some file
   * systems).
   *
  ...
  /**
   * Returns the canonicalized path obtained from the concatenation of the given string path to the
   * end of this path. The given string path must be a valid path. If it has a trailing separator,
   * the result will have a trailing separator. The device id of this path is preserved (the one of
   * the given string is ignored). Duplicate slashes are removed from the path except at the
  ...
  /**
   * Removes duplicate slashes from the given path, with the exception of leading double slash which
   ...
   *
   * <p>Equality for paths is defined to be: same sequence of segments, same absolute/relative
   * status, and same device. Trailing separators are disregarded. Paths are not generally
  ...
  /**
   * Returns whether the given string is syntactically correct as a path. The device id is the
   * prefix up to and including the device separator for the local file system; the path proper is
   * everything to the right of it, or the entire string if there is no device separator. When the
   * platform location is a file system with no meaningful device separator, the entire string is
   * treated as the path proper. The device id is not checked for validity; the path proper is
   * correct if each of the segments in its canonicalized form is valid.
   *
   * @param path the path to check
   * @return <code>true</code> if the given string is a valid path, and <code>false</code> otherwise
  ...
  public static boolean isValidPath(String path) {
    ...
    for (int i = 0, max = test.segmentCount(); i < max; i++)
      if (!isValidSegment(test.segment(i))) return false;
    return true;
  }

  /**
   * Returns whether the given string is valid as a segment in a path. The rules for valid segments
   * are as follows:
   *
   * <ul>
   ...
   * @return <code>true</code> if the given path segment is valid, and <code>false</code> otherwise
   * @since 4.0.0-RC5
   */
   ...
   * be negative. The device id is preserved.
   *
   * <p>If this path has a trailing separator, the result will too (assuming there are some segments
   * left). If there is no trailing separator, the result will not have a trailing separator.
  ...
  public Path uptoSegment(int count) {
    if (count == 0) return new Path(device, NO_SEGMENTS, separators & (HAS_LEADING | IS_UNC));
   ...
   *
   * @param paths the input array of paths
   ...
   *
   * <p>For example we have three paths:
   *
   * <ul>
   *   <li>{@code /a/b/c}
   *   <li>{@code /a/b/d}
   ...
   * @throws NullPointerException in case if given {@code paths} array is null
   * @since 5.0.0
   */
