...
public class FuzzyMatches {
  ...
  private final Matcher FUZZY_SEPARATE =
      or(new PrefixMatcher(true), new CamelCaseMatcher(), new SubStringMatcher());
  private final Matcher FUZZY_CONTIGUOUS =
      or(new PrefixMatcher(true), new CamelCaseMatcher(), new ContiguousSubStringMatcher());
  ...
  private class CamelCaseMatcher implements Matcher {
    ...
    public List<Match> match(String word, String wordToMatch) {
      ...
      while (i < wordToMatch.length()
          && (result = matchCamelCase(word.toLowerCase(), wordToMatch, 0, i)) == null) {
    ...
    private List<Match> matchCamelCase(String word, String wordToMatch, int i, int j) {
      if (i == word.length()) {
        ...
        while (result == null
            && (nextUpperIndex = nextAnchor(wordToMatch, nextUpperIndex)) < wordToMatch.length()) {
  ...
  private class PrefixMatcher implements Matcher {
    ...
    public List<Match> match(String word, String wordToMatch) {
      ...
      return word.length() > 0
          ? Collections.singletonList(new Match(0, word.length()))
