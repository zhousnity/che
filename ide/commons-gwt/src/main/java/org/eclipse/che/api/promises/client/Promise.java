...
public interface Promise<V> extends Thenable<V> {
   ...
   * The action is added both to the original promise and the returned value, but the promises are
   ...
   *
   * @param onFulfilled the fulfillment action added. Function should return promise which will be
   *     evaluated to determine the state of the returned promise
   ...
   *
   * <p>The action is added both to the original promise and the returned value, but the promises
   * are not necessarily the same object.
   *
   * <p>This is shorthand for {@code promise.then(null, onRejected)}.
   *
   * <p>Note: the method name does not match the ES6 specification, obviously because <code>catch
   * </code> can't be used in Java.
   *
   * @param onRejected the rejection action added
   ...
   *
   * @param onRejected the rejection action added. Function should return promise which will be
   *     evaluated to determine the state of the returned promise.
