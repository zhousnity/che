...
public class WebSocket extends JavaScriptObject {
  ...
  /**
   * Creates a new WebSocket instance. WebSocket attempt to connect to their URL immediately upon
   * creation.
   *
  ...
  /**
   * Creates a WebSocket object. WebSocket attempt to connect to their URL immediately upon
   * creation.
   *
   * @param url WebSocket server URL
  ...
  /**
   * Closes the WebSocket connection. If the connection state is already {@link
   * MessageBus.ReadyState#CLOSED}, this method does nothing.
   */
   ...
   *
   * @return <code>true</code> if WebSockets are supported; <code>false</code> if they are not.
   */
  ...
  /**
   * Represents the number of bytes of UTF-8 text that have been queued using send() method.
   *
  ...
  /**
   * Sets the {@link org.eclipse.che.ide.websocket.events.ConnectionOpenedHandler} to be notified
   * when the WebSocket connection established.
   *
  ...
  /**
   * Sets the {@link org.eclipse.che.ide.websocket.events.ConnectionErrorHandler} to be notified
   * when there is any error in communication.
   *
