...
abstract class AbstractMessageBus implements MessageBus {
  ...
  private static final int RECONNECTION_PERIOD = 2 * 1000;
  ...
  private void handleConnectionClosure(final WebSocketClosedEvent event) {
    connectionClosedHandlers.dispatch(
        new ListenerManager.Dispatcher<ConnectionClosedHandler>() {
          @Override
  ...
  private void handleErrorConnection() {
    connectionErrorHandlers.dispatch(
        new ListenerManager.Dispatcher<ConnectionErrorHandler>() {
          @Override
   ...
   *
   * @return <code>true</code> if WebSocket is supported; <code>false</code> if it's not
   */
   ...
   */
  private void internalSend(String uuid, String message, RequestCallback callback)
   ...
   *
   * @param message text message
   * @throws WebSocketException throws if an any error has occurred while sending data, e.g.:
   *     WebSocket is not supported by browser, WebSocket connection is not opened
   */
   ...
   * @throws WebSocketException throws if an any error has occurred while sending data
   */
  private void internalSend(String uuid, String message, ReplyHandler callback)
      throws WebSocketException {
    checkWebSocketConnectionState();
  ...
  /**
   * Check WebSocket connection and throws {@link WebSocketException} if WebSocket connection is not
   * ready to use.
   *
  ...
  private class WsListener
      implements ConnectionOpenedHandler, ConnectionClosedHandler, ConnectionErrorHandler {
