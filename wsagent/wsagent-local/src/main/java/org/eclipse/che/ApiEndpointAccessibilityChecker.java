...
/**
 * Checks whether API is accessible from WS agent or not on app start to prevent usage of illegal
 * configuration.
 *
...
public class ApiEndpointAccessibilityChecker {
  ...
  @Inject
  public ApiEndpointAccessibilityChecker(
  ...
  public void start() {
    try {
      final HttpJsonResponse pingResponse =
    ...
    LOG.error(
        "The workspace agent has attempted to start, but it is unable to ping the Che server at "
            + apiEndpoint);
    LOG.error(
        "The workspace agent has been forcefully stopped. "
    ...
    try {
      LOG.info(
          "Workspace /etc/hosts: "
              + IoUtil.readAndCloseQuietly(new FileInputStream(new File("/etc/hosts"))));
    } catch (Exception e) {
