...
public class LocalVirtualFileAssertionHelper {
  ...
  public void assertThatIoFilesHaveSameContent(Path fileOne, Path fileTwo) throws IOException {
    assertArrayEquals(
        String.format("Same content expected for files %s and %s", fileOne, fileTwo),
        Files.toByteArray(getIoFile(fileOne)),
        Files.toByteArray(getIoFile(fileTwo)));
  }
  ...
  public void assertThatMetadataIoFilesHaveSameContent(Path fileOne, Path fileTwo)
      throws IOException {
    assertArrayEquals(
        String.format("Same content expected for files %s and %s", fileOne, fileTwo),
        Files.toByteArray(getMetadataIoFile(fileOne)),
        Files.toByteArray(getMetadataIoFile(fileTwo)));
  }

  public void assertThatMetadataIoFileHasContent(Path virtualFilePath, byte[] bytes)
      throws IOException {
  ...
  private File getMetadataIoFile(Path virtualFilePath) {
    ...
    if (virtualFilePath.isRoot()) {
      metadataFilePath =
          virtualFilePath.newPath(".vfs", "props", virtualFilePath.getName() + "_props");
    } else {
      metadataFilePath =
          virtualFilePath
              .getParent()
              .newPath(".vfs", "props", virtualFilePath.getName() + "_props");
    }
  ...
  private File getLockIoFile(Path virtualFilePath) {
    ...
    if (virtualFilePath.isRoot()) {
      ...
      lockFilePath =
          virtualFilePath.getParent().newPath(".vfs", "locks", virtualFilePath.getName() + "_lock");
    }
