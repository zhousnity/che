...
public class MemoryVirtualFileTest {
  ...
  @Test
  public void failsCopyFolderWhenTargetFolderContainsItemWithSameNameAndOverwritingIsDisabled()
      throws Exception {
    VirtualFile folder = getRoot().createFolder(generateFolderName());
    VirtualFile file = folder.createFile(generateFileName(), DEFAULT_CONTENT);
  ...
  @Test
  public void
      failsCopyFolderUnderNewNameWhenTargetFolderContainsItemWithSameNameAndOverwritingIsDisabled()
          throws Exception {
    VirtualFile folder = getRoot().createFolder(generateFolderName());
    VirtualFile file = folder.createFile(generateFileName(), DEFAULT_CONTENT);
  ...
  @Test
  public void failsCopyFolderWhenTargetFolderNeedBeOverwrittenButContainsLockedFile()
      throws Exception {
    VirtualFile folder = getRoot().createFolder(generateFolderName());
    VirtualFile file = folder.createFile(generateFileName(), DEFAULT_CONTENT);
  ...
  public void movesFolder() throws Exception {
    ...
    List<Path> originalTreePaths =
        originalTree.stream().map(VirtualFile::getPath).collect(toList());
    VirtualFile targetFolder = getRoot().createFolder(generateFolderName());
    ...
    while (originalPathIterator.hasNext() && movedIterator.hasNext()) {
      ...
      assertEquals(
          String.format("Properties of virtual file %s not copied properly", moved.getPath()),
          ImmutableMap.of("property" + i, "value" + i),
          moved.getProperties());
      if (moved.isFile()) {
        assertEquals(
            String.format("Content of file %s not copied properly", moved.getPath()),
            DEFAULT_CONTENT,
            moved.getContentAsString());
      }
      assertNull(
          String.format("Item %s must ne removed", originalPath), getRoot().getChild(originalPath));
      i++;
    }
  ...
  public void renamesFolder() throws Exception {
    ...
    assertEquals(
        ImmutableMap.of("property2", "value2"), getRoot().getChild(newFilePath).getProperties());
  ...
  public void createsFileWithStreamContent() throws Exception {
    VirtualFile folder = getRoot().createFolder("a/b/c");
    VirtualFile file =
        folder.createFile("new_file", new ByteArrayInputStream(DEFAULT_CONTENT_BYTES));
