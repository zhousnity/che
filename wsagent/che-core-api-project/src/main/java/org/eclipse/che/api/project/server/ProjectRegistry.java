...
public class ProjectRegistry {
  ...
  public ProjectRegistry(
      WorkspaceProjectsSyncer workspaceHolder,
   ...
   */
  RegisteredProject putProject(
      ProjectConfig config, FolderEntry folder, boolean updated, boolean detected)
      throws ServerException {

    final RegisteredProject project =
        new RegisteredProject(folder, config, updated, detected, this.projectTypeRegistry);
  ...
  void removeProjects(String path) throws ServerException {
    ...
    getProjects(path).forEach(p -> Optional.ofNullable(projects.remove(p)).ifPresent(removed::add));
  ...
  /**
   * Extension writer should call this method to apply changes which (supposedly) change Attributes
   * defined with particular Project Type If incoming Project Type is primary and: - If the folder
   * located on projectPath is a Project, its Primary PT will be converted to incoming PT - If the
   * folder located on projectPath is NOT a Project the folder will be converted to "detected"
   * Project with incoming Primary PT If incoming Project Type is mixin and: - If the folder located
   * on projectPath is a Project, this PT will be added (if not already there) to its Mixin PTs - If
   * the folder located on projectPath is NOT a Project - ConflictException will be thrown For
   * example: - extension code knows that particular file content is used by Value Provider so this
   * method should be called when content of this file changed to check and update attributes. OR If
   * Extension writer wants to force initializing folder to be Project For example: - extension code
   * knows that particular folder inside should (or may) be treated as sub-project of same as
   * "parent" project type
   *
   ...
   * @throws ServerException
   */
  public RegisteredProject setProjectType(String projectPath, String type, boolean asMixin)
  ...
  /**
   * Extension writer should call this method to apply changes which supposedly make the Project no
   * longer have particular Project Type. In a case of removing primary project type: - if the
   * project was NOT detected BASE Project Type will be set as primary - if the project was detected
   * it will be converted back to the folder For example: - extension code knows that removing some
   * file inside project's file system will (or may) cause removing particular project type
   *
   * @param projectPath project path
   ...
   * @throws ServerException
   */
  public RegisteredProject removeProjectType(String projectPath, String type)
      throws ConflictException, ForbiddenException, NotFoundException, ServerException {
    final RegisteredProject project = getProject(projectPath);
