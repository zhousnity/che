...
public interface SearcherProvider {
   ...
   * @param create {@code true} to create new Searcher if necessary; {@code false} to return {@code
   *     null} if Searcher is not initialized yet
   * @return {@code Searcher} or {@code null} if {@code create} is {@code false} and the Searcher is
   *     not initialized yet
  ...
  /**
   * Get Searcher for specified VirtualFileSystem. This method is shortcut for {@code
   * getSearcher(VirtualFileSystem, true)}.
   *
   * @param virtualFileSystem VirtualFileSystem
