...
/**
 * Provide possibility for importing source from some resource e.g. VCS (like Git or SVN) or from
 * ZIP archive
 *
...
public interface ProjectImporter {
   ...
   * @param storage the object which contains information about source(location, parameters, type
   *     etc.)
   ...
   */
  void importSources(FolderEntry baseFolder, SourceStorage storage)
   ...
   * @param storage the object which contains information about source(location, parameters, type
   *     etc.)
   * @param importOutputConsumerFactory an optional output line consumer factory to get the import
   *     process output. For instance, Git command output for the Git importer
   * @throws ForbiddenException if some operations in {@code baseFolder} are forbidden, e.g. current
   *     user doesn't have write permissions to the {@code baseFolder}
   * @throws ConflictException if import causes any conflicts, e.g. if import operation causes name
   *     conflicts in {@code baseFolder}
