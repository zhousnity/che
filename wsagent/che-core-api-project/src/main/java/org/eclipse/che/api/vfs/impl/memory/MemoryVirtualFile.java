...
public class MemoryVirtualFile implements VirtualFile {
  ...
  static MemoryVirtualFile newFile(MemoryVirtualFile parent, String name, InputStream content)
      throws IOException {
    return new MemoryVirtualFile(
        parent, name, content == null ? new byte[0] : ByteStreams.toByteArray(content));
  }

  static MemoryVirtualFile newFile(MemoryVirtualFile parent, String name, byte[] content) {
    return new MemoryVirtualFile(
        parent, name, content == null ? new byte[0] : Arrays.copyOf(content, content.length));
  }
  ...
  @Override
  public VirtualFile updateProperties(Map<String, String> properties)
      throws ForbiddenException, ServerException {
    return updateProperties(properties, null);
  }

  @Override
  public VirtualFile setProperty(String name, String value, String lockToken)
      throws ForbiddenException, ServerException {
  ...
  @Override
  public VirtualFile setProperty(String name, String value)
      throws ForbiddenException, ServerException {
  ...
  public byte[] getContentAsBytes() throws ForbiddenException {
    ...
    throw new ForbiddenException(
        String.format(
            "We were unable to retrieve the content. Item '%s' is not a file", getPath()));
  }
  ...
  @Override
  public VirtualFile updateContent(InputStream content, String lockToken)
      throws ForbiddenException, ServerException {
    byte[] bytes;
    try {
      ...
      throw new ServerException(
          String.format(
              "We were unable to set the content of '%s'. Error: %s", getPath(), e.getMessage()));
    }
  ...
  @Override
  public VirtualFile updateContent(byte[] content, String lockToken)
      throws ForbiddenException, ServerException {
  ...
  private void doUpdateContent(byte[] content, String lockToken)
      throws ForbiddenException, ServerException {
    checkExistence();
  ...
  @Override
  public VirtualFile copyTo(VirtualFile parent)
  ...
  @Override
  public VirtualFile moveTo(VirtualFile parent)
      throws ForbiddenException, ConflictException, ServerException {
  ...
  public VirtualFile rename(String newName, String lockToken)
    ...
    if (isFolder) {
      ...
      if (!lockedFiles.isEmpty()) {
        throw new ForbiddenException(
            String.format(
                "Unable rename item '%s'. Child items '%s' are locked", getName(), lockedFiles));
      }
    } else {
      if (fileIsLockedAndLockTokenIsInvalid(lockToken)) {
        throw new ForbiddenException(
            String.format(
                "We were unable to rename an item '%s'."
                    + " The item is currently locked by the system",
                getPath()));
      }
    }
  ...
  @Override
  public VirtualFile rename(String newName)
      throws ForbiddenException, ConflictException, ServerException {
  ...
  public void delete(String lockToken) throws ForbiddenException, ServerException {
    ...
    if (folder) {
      ...
      if (!lockedFiles.isEmpty()) {
        throw new ForbiddenException(
            String.format(
                "Unable delete item '%s'. Child items '%s' are locked", getName(), lockedFiles));
      }
      ...
      if (fileIsLockedAndLockTokenIsInvalid(lockToken)) {
        throw new ForbiddenException(
            String.format("Unable delete item '%s'. Item is locked", getPath()));
      }
    }
    parent.children.remove(name);
    exists = false;
  ...
  public InputStream zip() throws ForbiddenException, ServerException {
    ...
    if (isFolder()) {
      ...
      throw new ForbiddenException(
          String.format("Unable export to zip. Item '%s' is not a folder", getPath()));
    }
  }
  ...
  public InputStream tar() throws ForbiddenException, ServerException {
    ...
    if (isFolder()) {
      ...
      throw new ForbiddenException(
          String.format("Unable export to tar archive. Item '%s' is not a folder", getPath()));
    }
  }
  ...
  public void untar(InputStream tarArchive, boolean overwrite, int stripNumber)
    ...
    if (isFolder()) {
      extract(
          fileSystem.getArchiverFactory().createArchiver(this, "tar"),
          tarArchive,
          overwrite,
          stripNumber);
      addInSearcher(this);
      ...
      throw new ForbiddenException(
          String.format("Unable import tar archive. Item '%s' is not a folder", getPath()));
    }
  }
  ...
  @Override
  public VirtualFile createFile(String name, byte[] content)
      throws ForbiddenException, ConflictException, ServerException {
  ...
  @Override
  public VirtualFile createFolder(String name)
      throws ForbiddenException, ConflictException, ServerException {
    checkExistence();
    checkName(name);
    if (name.charAt(0) == '/') {
    ...
    if (isFolder()) {
      ...
      if (name.indexOf('/') > 0) {
        ...
        if (newFolder == null) {
          throw new ConflictException(
              String.format("Item with the name '%s' already exists", name));
        }
      } else {
      ...
      throw new ForbiddenException(
          "Unable create new folder. Item specified as parent is not a folder");
    }
  }
