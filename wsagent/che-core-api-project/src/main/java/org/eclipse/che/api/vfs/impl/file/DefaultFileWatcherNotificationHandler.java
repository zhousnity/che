...
public class DefaultFileWatcherNotificationHandler implements FileWatcherNotificationHandler {
  ...
  @Inject
  public DefaultFileWatcherNotificationHandler(
  ...
  @Override
  public boolean addNotificationListener(
  ...
  private VirtualFile convertToVirtualFile(File root, String subPath, boolean isDir) {
    try {
      LocalVirtualFileSystem virtualFileSystem =
          (LocalVirtualFileSystem) virtualFileSystemProvider.getVirtualFileSystem(true);
      Path vfsPath = Path.of(subPath);
      VirtualFile virtualFile = virtualFileSystem.getRoot().getChild(vfsPath);
      if (virtualFile == null) {
        virtualFile =
            new DeletedLocalVirtualFile(
                new File(root, subPath), ROOT.newPath(vfsPath), virtualFileSystem, isDir);
      }
      return virtualFile;
  ...
  private static class DeletedLocalVirtualFile extends LocalVirtualFile {
    ...
    DeletedLocalVirtualFile(
        File ioFile, Path path, LocalVirtualFileSystem fileSystem, boolean isDir) {
