...
public final class Path {
  ...
  private static String[] splitToSegments(String rawPath) {
    return (isNullOrEmpty(rawPath) || ((rawPath.length() == 1) && (rawPath.charAt(0) == '/')))
        ? EMPTY_PATH
        : PATH_SPLITTER.split(rawPath.charAt(0) == '/' ? rawPath.substring(1) : rawPath);
  }
      ...
      if ("..".equals(segment)) {
        ...
        if (size == 0) {
          throw new IllegalArgumentException(
              String.format("Invalid path '%s', '..' on root. ", Joiner.on('/').join(relative)));
        }
