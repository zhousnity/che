...
public class OutputTestListener extends AbstractTestListener implements AutoCloseable {
  ...
  private static final String consumeErrorMessage =
      "An exception occured while trying to send a 'TestingOutput' "
          + "object through web sockets on the following channel: "
          + TESTING_OUTPUT_CHANNEL_NAME;

  private WebsocketMessageConsumer<TestingOutput> consumer =
      new WebsocketMessageConsumer<>(TESTING_OUTPUT_CHANNEL_NAME);
  ...
  private void addProblem(String testKey, Throwable throwable, boolean isError) {
    ...
    TestingOutput.LineType lineType =
        isError ? TestingOutput.LineType.ERROR : TestingOutput.LineType.FAILURE;
