...
/**
 * Reads project template descriptions that may be described in separate json-files for every
 * project type. This file should be named as &lt;project_type_id&gt;.json.
 *
...
public class ProjectTemplateDescriptionLoader {
  ...
  /**
   * @param templateDescriptionLocationDir Describes value which is a path the directory with
   *     template descriptors (not sources).
   * @param templateRegistry registry which contains templates associated with tags
   */
  @Inject
  public ProjectTemplateDescriptionLoader(
      @Named("che.template.storage") String templateDescriptionLocationDir,
  ...
  @PostConstruct
  public void start() {
    if (templateDescriptionLocationDir == null
        || !Files.exists(Paths.get(templateDescriptionLocationDir))
  ...
  private void resolveTemplate(InputStream stream) throws IOException {
    final List<ProjectTemplateDescriptor> templates =
        DtoFactory.getInstance().createListDtoFromJson(stream, ProjectTemplateDescriptor.class);
