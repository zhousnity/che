...
public interface Instance extends Machine {
   ...
   * @throws NotFoundException if process with specified id is not found. Process can be finished
   ...
   * @throws MachineException if any error occurs on the processes list retrieving
   */
  List<InstanceProcess> getProcesses() throws MachineException;

  /**
   * Create process from command line. Returned {@link InstanceProcess#getPid()} should return
   * unique pid on this stage. This pid allow to control process from clients and save process logs
   * if needed.
   *
   * @param command command from which process should be created
   * @param outputChannel websocket chanel for execution logs
   * @return {@link InstanceProcess} with unique pid, that can't be used in future for other process
   *     of instance
   * @throws MachineException if error occurs on creating process
   */
   ...
   * @throws MachineException if error occurs on storing state of the instance
   */
   ...
   * Destroy instance
   *
   * @throws MachineException if error occurs on instance destroying
   */
   ...
   * @param sourcePath path to file or directory inside specified machine
   * @param targetPath path to destination file or directory inside machine
   * @param overwriteDirNonDir If "false" then it will be an error if unpacking the given content
   *     would cause an existing directory to be replaced with a non-directory and vice versa.
   ...
   *
   * @param sourcePath path to file or directory inside CHE server
   * @param targetPath path to destination file or directory inside machine
   * @throws MachineException if any error occurs when files are being copied
   */
