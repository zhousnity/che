...
public class MachineConfigImpl implements MachineConfig {
  ...
  public MachineConfigImpl(
    ...
    if (servers != null) {
      this.servers = servers.stream().map(ServerConfImpl::new).collect(Collectors.toList());
    }
    if (source != null) {
  ...
  public static class MachineConfigImplBuilder {
    ...
    public MachineConfigImpl build() {
      return new MachineConfigImpl(dev, name, type, source, machineLimits, servers, envVariables);
    }
