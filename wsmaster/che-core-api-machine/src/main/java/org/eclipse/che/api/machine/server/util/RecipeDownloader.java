...
public class RecipeDownloader {
   ...
   *
   * @param machineConfig config used to get recipe location
  ...
  public RecipeImpl getRecipe(MachineConfig machineConfig) throws MachineException {
    ...
    try {
      ...
      return new RecipeImpl()
          .withType(machineConfig.getSource().getType())
          .withScript(IoUtil.readAndCloseQuietly(new FileInputStream(file)));
    } catch (IOException | IllegalArgumentException e) {
      throw new MachineException(
          format(
              "Failed to download recipe for machine %s. Recipe url %s. Error: %s",
              machineConfig.getName(), location, e.getLocalizedMessage()));
    } finally {
