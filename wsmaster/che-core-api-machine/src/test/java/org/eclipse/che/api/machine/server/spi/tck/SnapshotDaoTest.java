...
public class SnapshotDaoTest {
  ...
  @Inject private TckRepository<SnapshotImpl> snaphotRepo;

  @Inject private TckRepository<Workspace> workspaceRepo;
  ...
  public void shouldGetSnapshotByWorkspaceEnvironmentAndMachineName() throws Exception {
    ...
    assertEquals(
        snapshotDao.getSnapshot(
            snapshot.getWorkspaceId(), snapshot.getEnvName(), snapshot.getMachineName()),
        snapshot);
  }

  @Test(expectedExceptions = NotFoundException.class, dataProvider = "missingSnapshots")
  public void shouldThrowNotFoundExceptionWhenSnapshotMissing(
      String wsId, String envName, String machineName) throws Exception {
  ...
  public void shouldThrowNpeWhenAnyOfGetSnapshotParametersIsNull(
      String wsId, String envName, String machineName) throws Exception {
    snapshotDao.getSnapshot(wsId, envName, machineName);
  }
  ...
  public void shouldFindSnapshotsByWorkspaceAndNamespace() throws Exception {
    ...
    assertEquals(
        new HashSet<>(found), new HashSet<>(asList(snapshots[0], snapshots[1], snapshots[2])));
  }
  ...
  public void shouldSaveSnapshot() throws Exception {
    final SnapshotImpl newSnapshot =
        createSnapshot("new-snapshot", workspaces[0].getId(), "env-name", "machine-name");
  ...
  @Test(expectedExceptions = SnapshotException.class)
  public void shouldNotSaveSnapshotForMachineIfSnapshotForSuchMachineAlreadyExists()
      throws Exception {
    final SnapshotImpl snapshot = snapshots[0];
  ...
  public void replacesSnapshots() throws Exception {
    ...
    final List<SnapshotImpl> replaced =
        snapshotDao.replaceSnapshots(
            newSnapshot.getWorkspaceId(), newSnapshot.getEnvName(), singletonList(newSnapshot));

    assertEquals(new HashSet<>(replaced), Sets.newHashSet(snapshots[0], snapshots[1]));
    final HashSet<SnapshotImpl> actual =
        new HashSet<>(snapshotDao.findSnapshots(this.snapshots[0].getWorkspaceId()));
  ...
  private static SnapshotImpl createSnapshot(
      String id, String workspaceId, String envName, String machineName) {
    return SnapshotImpl.builder()
        .setId(id)
        .setType(id + "type")
        .setMachineSource(
            new MachineSourceImpl(
                id + "source-type", id + "source-location", id + "source-content"))
