...
public class CheEnvironmentEngineTest {
  ...
  @Mock private MessageConsumer<MachineLogMessage> messageConsumer;
  @Mock private InstanceProvider instanceProvider;
  @Mock private MachineInstanceProvider machineProvider;
  ...
  @Mock private MachineStartedHandler startedHandler;
  @Mock private WorkspaceSharedPool sharedPool;

  @Captor ArgumentCaptor<EventSubscriber<InstanceStateEvent>> eventServiceSubscriberCaptor;
  ...
  @Test(
    expectedExceptions = EnvironmentNotRunningException.class,
  ...
  @Test(
    ...
    expectedExceptionsMessageRegExp =
        "Machine with ID .* is not found in the environment of workspace .*"
  )
  ...
  public void stopsTheEnvironmentWhileStartOfMachineIsInterrupted() throws Exception {
    ...
    when(machineProvider.startService(
        ...
        .thenAnswer(
            invocationOnMock -> {
              ...
              Object[] arguments = invocationOnMock.getArguments();
              NoOpMachineInstance instance =
                  spy(
                      new NoOpMachineInstance(
                          createMachine(
                              workspaceId,
                              envName,
    ...
    try {
      ...
      assertEquals(
          x.getMessage(),
          format(
              "Start of environment '%s' in workspace '%s' is interrupted", envName, workspaceId));
    }
  ...
  public void shouldBeAbleToStartEnvironmentWithRecover() throws Exception {
    ...
    MachineSourceImpl machineSource =
        new MachineSourceImpl(
            "image", "registry.com/snapshot123:latest@sha256:abc1234567890", null);
  ...
  public void shouldBeAbleToStartEnvironmentWhenRecoverFailed() throws Exception {
    ...
    when(snapshotDao.getSnapshot(anyString(), anyString(), anyString()))
        .thenThrow(new NotFoundException("Snapshot not found"));
  ...
  @Test
  public void shouldSetDockerfileContentInsteadOfUrlIfUrlPointsToCheApiOnMachineStart()
      throws Exception {
    // given
    List<Instance> instances = startEnv();
    String workspaceId = instances.get(0).getWorkspaceId();
  ...
  @Test
  public void shouldNotSetDockerfileContentInsteadOfUrlIfUrlDoesNotPointToCheApiOnMachineStart()
      throws Exception {
    // given
    List<Instance> instances = startEnv();
    String workspaceId = instances.get(0).getWorkspaceId();
  ...
  public void shouldApplyAgentsOnDockerMachineStart() throws Exception {
    ...
    // then
    verify(infrastructureProvisioner)
        .provision(any(ExtendedMachineImpl.class), any(CheServiceImpl.class));
  }
  ...
  @Test(
    expectedExceptions = EnvironmentNotRunningException.class,
    expectedExceptionsMessageRegExp =
        "Stop of not running environment of workspace with ID '.*' is not allowed."
  )
  ...
  public void shouldBeAbleToStartNonDockerMachine() throws Exception {
    ...
    doReturn(newMachine)
        .when(instanceProvider)
        .createInstance(any(Machine.class), any(LineConsumer.class));

    MachineConfigImpl config =
        MachineConfigImpl.builder().fromConfig(createConfig(false)).setType("anotherType").build();
  ...
  public void shouldBeAbleToSaveMachineSnapshot() throws Exception {
    ...
    List<Instance> instances = startEnv();
    Instance instance = instances.get(0);
    doReturn(new MachineSourceImpl("someType").setContent("some content"))
        .when(instance)
        .saveToSnapshot();
  ...
  public void shouldReplaceServiceNameWithContainerNameAndUseAliasInLinks() {
    ...
    assertEquals(serviceToNormalizeLinks.getLinks().size(), 1);
    assertEquals(
        serviceToNormalizeLinks.getLinks().get(0),
        containerNameToLink + ':' + AliasToServiceToLink);
  }
  ...
  public void shouldBeAbleToRestartEnvironmentAfterAgentException() throws Exception {
    ...
    doThrow(new AgentException(testAgentExcMessage))
        .when(startedHandler)
  ...
  private EnvironmentImpl createEnv() {
    ...
    String environmentRecipeContent =
        "services:\n  "
            ...
            + "machine2:\n    image: codenvy/ubuntu_jdk8\n    mem_limit: 100000";
    env.setRecipe(
        new EnvironmentRecipeImpl("compose", "application/x-yaml", environmentRecipeContent, null));
