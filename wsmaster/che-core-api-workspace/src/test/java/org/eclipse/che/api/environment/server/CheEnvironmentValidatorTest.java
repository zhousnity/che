...
public class CheEnvironmentValidatorTest {
  @Mock MachineInstanceProviders machineInstanceProviders;
  @Mock EnvironmentParser environmentParser;
  @Mock DefaultServicesStartStrategy startStrategy;

  @InjectMocks CheEnvironmentValidator environmentValidator;
  ...
  @Test(
    ...
    expectedExceptionsMessageRegExp =
        "Parsing of recipe of environment '.*' failed. Error: test exception"
  )
  ...
  @Test(
    expectedExceptions = IllegalArgumentException.class,
    expectedExceptionsMessageRegExp =
        "Type 'compose' of environment 'env' is not supported. Supported types: otherType"
  )
  ...
  @Test(
    expectedExceptions = IllegalArgumentException.class,
    expectedExceptionsMessageRegExp =
        "Start order of machine in environment 'env' is not resolvable. Error: test exception"
  )
  ...
  public static Object[][] invalidEnvironmentProvider() {
    ...
    data.add(
        asList(
            env, "Type 'docker' of environment 'env' is not supported. Supported types: compose"));
    ...
    data.add(
        asList(
            env,
            "Recipe of environment 'env' contains mutually exclusive fields location and content"));
    ...
    data.add(
        asList(
            env,
            "Environment 'env' doesn't contain machine with 'org.eclipse.che.ws-agent' agent"));
    ...
    data.add(
        asList(
            env,
            "Environment 'env' contains machines that are missing in environment recipe: missingInEnvMachine"));

    env = createEnv();
    env.getMachines()
        .entrySet()
        .forEach(entry -> entry.getValue().getAgents().add("org.eclipse.che.ws-agent"));
    data.add(
        asList(
            env,
            "Environment 'env' should contain exactly 1 machine with agent 'org.eclipse.che.ws-agent', but contains '"
                + env.getMachines().size()
                + "'. "
                + "All machines with this agent: "
    ...
    data.add(
        asList(
            env,
            format(
                "Value of attribute 'memoryLimitBytes' of machine '%s' in environment 'env' is illegal",
                machineEntry.getKey())));
  ...
  public void shouldNotFailIfExtraMachineDoesNotHaveExtendedMachineEntry() throws Exception {
    ...
    cheServicesEnv
        .getServices()
        .put("extra", createCheService("_extra", 1000000L, null, null, null));
    when(environmentParser.parse(any(Environment.class))).thenReturn(cheServicesEnv);
  ...
  public void shouldFailValidationIfCheServicesEnvironmentIsBroken(
      CheServicesEnvironmentImpl cheServicesEnv, String expectedExceptionMessage) throws Exception {
  ...
  public static Object[][] invalidServicesEnvironmentProvider() {
    ...
    data.add(
        asList(
            env,
            format(
                "Field 'image' or 'build.context' is required in machine '%s' in environment 'env'",
                serviceEntry.getKey())));
    ...
    data.add(
        asList(
            env,
            format(
                "Ports binding is forbidden but found in machine '%s' of environment 'env'",
                serviceEntry.getKey())));
    ...
    data.add(
        asList(
            env,
            format(
                "Volumes binding is forbidden but found in machine '%s' of environment 'env'",
                serviceEntry.getKey())));
    ...
    data.add(
        asList(
            env,
            format(
                "Networks configuration is forbidden but found in machine '%s' of environment 'env'",
                serviceEntry.getKey())));
    ...
    data.add(
        asList(
            env,
            format(
                "Machine '%s' in environment 'env' contains mutually exclusive dockerfile content and build context.",
                serviceEntry.getKey())));
  ...
  private static Map.Entry<String, CheServiceImpl> getAnyService(CheServicesEnvironmentImpl env) {
    return env.getServices().entrySet().iterator().next();
  }
  ...
  @Test(
    expectedExceptions = IllegalArgumentException.class,
    expectedExceptionsMessageRegExp =
        "Type 'null' of machine '.*' is not supported. Supported values are: docker, ssh."
  )
  ...
  @Test(
    expectedExceptions = IllegalArgumentException.class,
    expectedExceptionsMessageRegExp =
        "Type 'compose' of machine '.*' is not supported. Supported values are: docker, ssh."
  )
  ...
  @Test(
    expectedExceptions = IllegalArgumentException.class,
    expectedExceptionsMessageRegExp =
        "Machine '.*' contains environment variable 'key' with null value"
  )
  ...
  public void shouldFailValidationIfLocationIsInvalidUrl() throws Exception {
    MachineConfigDto config = createMachineConfig();
    config.withSource(
        newDto(MachineSourceDto.class).withType("dockerfile").withLocation("localhost"));
  ...
  public void shouldFailValidationIfServerConfProtocolIsInvalid(String invalidProtocol)
      throws Exception {
    MachineConfigDto config = createMachineConfig();
    config
        .getServers()
        .add(newDto(ServerConfDto.class).withPort("8080/tcp").withProtocol(invalidProtocol));
  ...
  private static EnvironmentDto createEnv() {
    ...
    servers.put(
        "ref1", new ServerConf2Impl("8080/tcp", "proto1", singletonMap("prop1", "propValue")));
    ...
    machines.put(
        ...
        new ExtendedMachineImpl(
            asList("org.eclipse.che.ws-agent", "someAgent"),
            servers,
            singletonMap("memoryLimitBytes", "10000")));
    machines.put(
        "machine2",
        new ExtendedMachineImpl(
            asList("someAgent2", "someAgent3"), null, singletonMap("memoryLimitBytes", "10000")));
