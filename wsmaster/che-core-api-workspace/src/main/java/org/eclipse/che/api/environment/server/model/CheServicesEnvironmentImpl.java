    ...
    if (environment.getServices() != null) {
      services =
          environment
              ...
              .entrySet()
              .stream()
              .collect(toMap(Map.Entry::getKey, entry -> new CheServiceImpl(entry.getValue())));
    }
  }

  /**
   * ID of workspace to which this environment belongs. May be used for internal purposes of
