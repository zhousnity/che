...
public class WorkspaceConfigMessageBodyAdapter implements MessageBodyAdapter {
  ...
  /**
   * Matches those string which contain substring like <b>"environments" : [</b>. Old format defines
   * "environments" as json array while new format defines "environments" as json object.
   */
  private static final Pattern CONTAINS_ENVIRONMENTS_ARRAY_PATTERN =
      Pattern.compile(".*\"environments\"\\s*:\\s*\\[.*", DOTALL);

  @Inject protected WorkspaceConfigJsonAdapter configAdapter;
