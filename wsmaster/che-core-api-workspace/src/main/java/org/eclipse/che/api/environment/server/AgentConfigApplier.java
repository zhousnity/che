...
/**
 * Applies docker specific properties of the agents to {@link CheServiceImpl} or {@link
 * CheServicesEnvironmentImpl}. Dependencies between agents are respected. This class must be called
 * before machines represented by {@link CheServiceImpl} is started, otherwise changing
...
public class AgentConfigApplier {
   ...
   *
   * @param envConfig environment config with the list of agents that should be injected into
   *     machine
   * @param internalEnv affected environment of machines
   * @throws AgentException if any error occurs
   */
  public void apply(Environment envConfig, CheServicesEnvironmentImpl internalEnv)
      throws AgentException {
    for (Map.Entry<String, ? extends ExtendedMachine> machineEntry :
   ...
   *
   * @param machineConf machine config with the list of agents that should be injected into machine
   * @param machine affected machine
   * @throws AgentException if any error occurs
   */
  public void apply(@Nullable ExtendedMachine machineConf, CheServiceImpl machine)
      throws AgentException {
    if (machineConf != null) {
