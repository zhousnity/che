...
public class CheEnvironmentEngine {
   ...
   *
   * @param workspaceId ID of workspace that owns environment machines
   * @return list of machines
   * @throws EnvironmentNotRunningException if environment is not running
   */
    ...
    try (@SuppressWarnings("unused")
      ...
      if (environment == null) {
        throw new EnvironmentNotRunningException(
            "Environment with ID '" + workspaceId + "' is not found");
      }
   ...
   * @throws EnvironmentNotRunningException if environment is not running
   * @throws NotFoundException if machine is not found in the environment
   */
        ...
        .orElseThrow(
            () ->
                new NotFoundException(
                    format(
                        "Machine with ID '%s' is not found in the environment of workspace '%s'",
   ...
   *
   * <p>Environment starts if and only all machines in environment definition start successfully.
   * <br>
   * Otherwise exception is thrown by this method.<br>
   * It is not defined whether environment start fails right after first failure or in the end of
   * the process.<br>
   * Starting order of machines is not guarantied. Machines can start sequentially or in parallel.
   *
   * @param workspaceId ID of workspace that owns provided environment
   * @param envName name of environment
   * @param env environment to start
   * @param recover whether machines from environment should be recovered or not
   * @param messageConsumer consumer of log messages from machines in the environment
   * @param startedHandler handler for started machines
   * @return list of running machines of this environment
   * @throws ServerException if other error occurs
   */
  public List<Instance> start(
      String workspaceId,
      String envName,
      Environment env,
      boolean recover,
      ...
      throws ServerException, EnvironmentException, AgentException, ConflictException {
    ...
    initializeEnvironment(ownerName, workspaceId, envName, environment, networkId, messageConsumer);

    String devMachineName = getDevMachineName(environment);
    if (devMachineName == null) {
      throw new ServerException(
          "Agent 'org.eclipse.che.ws-agent' is not found in any of environment machines");
    }

    startEnvironmentQueue(
        ownerName, workspaceId, devMachineName, networkId, recover, startedHandler);

    try (@SuppressWarnings("unused")
        Unlocker u = stripedLocks.writeLock(workspaceId)) {
  ...
  public List<Instance> start(
      ...
      boolean recover,
      MessageConsumer<MachineLogMessage> messageConsumer)
      throws ServerException, ConflictException, AgentException, EnvironmentException {
   ...
   * @throws EnvironmentNotRunningException when environment is not running
   * @throws ServerException if other error occurs
   */
  public void stop(String workspaceId) throws EnvironmentNotRunningException, ServerException {
    ...
    try (@SuppressWarnings("unused")
      ...
      if (environmentHolder == null || environmentHolder.status != EnvStatus.RUNNING) {
        throw new EnvironmentNotRunningException(
            format(
                "Stop of not running environment of workspace with ID '%s' is not allowed.",
                workspaceId));
      }
   ...
   *
   * @param workspaceId ID of workspace that owns environment in which machine should be started
   * @param machineConfig configuration of machine that should be started
   * @return running machine
   * @throws EnvironmentNotRunningException if environment is not running
   * @throws NotFoundException if provider of machine implementation is not found
   * @throws ConflictException if machine with the same name already exists in the environment
   * @throws ServerException if any other error occurs
   */
  public Instance startMachine(String workspaceId, MachineConfig machineConfig, List<String> agents)
      throws ServerException, NotFoundException, ConflictException, EnvironmentException {
    ...
    if ("docker".equals(machineConfig.getType())) {
      ...
      normalize(ownerName, workspaceId, machineConfig.getName(), service);
      machine.setId(service.getId());

      machineStarter =
          (machineLogger, machineSource) -> {
            CheServiceImpl serviceWithNormalizedSource =
                normalizeServiceSource(service, machineSource);

            normalize(ownerName, workspaceId, machineConfig.getName(), serviceWithNormalizedSource);

            infrastructureProvisioner.provision(
                new ExtendedMachineImpl().withAgents(agents), serviceWithNormalizedSource);

            return machineProvider.startService(
                ownerName,
                workspaceId,
      ...
      try {
        ...
        throw new NotFoundException(
            format("Provider of machine type '%s' not found", machineConfig.getType()));
      }
    }
   ...
   *
   * @param workspaceId ID of workspace of environment that owns machine
   * @param machineId ID of machine that should be stopped
   * @throws NotFoundException if machine in not found in environment
   * @throws EnvironmentNotRunningException if environment is not running
   * @throws ConflictException if stop of dev machine is requested
   * @throws ServerException if other error occurs
   */
  public void stopMachine(String workspaceId, String machineId)
    ...
    if (targetMachine == null) {
      throw new NotFoundException(
          format(
              "Machine with ID '%s' is not found in environment of workspace '%s'",
              machineId, workspaceId));
    }
   ...
   * @return snapshot
   * @throws EnvironmentNotRunningException if environment of machine is not running
   * @throws NotFoundException if machine is not running
   * @throws ServerException if another error occurs
   */
  public SnapshotImpl saveSnapshot(String workspaceId, String machineId)
   ...
   *
   * @param snapshot description of snapshot that should be removed
   * @throws NotFoundException if snapshot is not found
   * @throws ServerException if error occurs on snapshot removal
   */
  ...
  private void initializeEnvironment(
      ...
      String networkId,
      MessageConsumer<MachineLogMessage> messageConsumer)
      throws ServerException, ConflictException, EnvironmentException {
    ...
    normalize(ownerName, workspaceId, internalEnv);
    ...
    try (@SuppressWarnings("unused")
      ...
      if (environments.putIfAbsent(workspaceId, environmentHolder) != null) {
        throw new ConflictException(
            format("Environment of workspace '%s' already exists", workspaceId));
      }
    }
  }
  ...
  private void normalize(
      String ownerName, String workspaceId, CheServicesEnvironmentImpl environment)
      throws ServerException {
    ...
    for (Map.Entry<String, CheServiceImpl> serviceEntry : services.entrySet()) {
      normalize(ownerName, workspaceId, serviceEntry.getKey(), serviceEntry.getValue());
    }
  }
   ...
   *
   * @param environment environment in which names will be normalized
   */
  ...
  /**
   * Replaces linked to this service's name with container name which represents the service in
   ...
   *
   * @param serviceToNormalizeLinks service which links will be normalized
   * @param services all services in environment
   */
  @VisibleForTesting
  void normalizeLinks(
      CheServiceImpl serviceToNormalizeLinks, Map<String, CheServiceImpl> services) {
    serviceToNormalizeLinks.setLinks(
        serviceToNormalizeLinks
            .getLinks()
            .stream()
            .map(
                link -> {
                  ...
                  String serviceAlias =
                      (serviceNameAndAliasToLink.length > 1) ? serviceNameAndAliasToLink[1] : null;
  ...
  private void normalize(
      String ownerName, String workspaceId, String machineName, CheServiceImpl service)
      throws ServerException {
    ...
    service.setContainerName(
        containerNameGenerator.generateContainerName(
            workspaceId, service.getId(), ownerName, machineName));
  }

  /** Starts all machine from machine queue of environment. */
  private void startEnvironmentQueue(
      String ownerName,
      String workspaceId,
      ...
      boolean recover,
      MachineStartedHandler startedHandler)
      throws ServerException, AgentException, EnvironmentException {
    ...
    try {
      ...
      while (machineName != null) {
        ...
        if (service == null) {
          LOG.error(
              "Start of machine with name {} in workspace {} failed. Machine not found in start queue",
              machineName,
              workspaceId);
          throw new ServerException(
              format(
                  "Environment of workspace with ID '%s' failed due to internal error",
                  workspaceId));
        }
            ...
            MachineImpl.builder()
                .setConfig(
                    MachineConfigImpl.builder()
                        .setDev(isDev)
                        .setLimits(new MachineLimitsImpl(bytesToMB(service.getMemLimit())))
                        .setType("docker")
                        .setName(machineName)
  ...
  private void checkInterruption(String workspaceId, String envName)
      throws EnvironmentStartInterruptedException {
  ...
  private Instance startInstance(
    ...
    try {
      ...
      machineLogger =
          getMachineLogger(environmentLogger, machine.getId(), machine.getConfig().getName());
        ...
        if (recover) {
          try {
            SnapshotImpl snapshot =
                snapshotDao.getSnapshot(
                    machine.getWorkspaceId(), machine.getEnvName(), machine.getConfig().getName());
  ...
  private interface MachineStarter {
    Instance startMachine(LineConsumer machineLogger, MachineSource machineSource)
        throws ServerException, NotFoundException, EnvironmentException;
  }
  ...
  private void addMachine(MachineImpl machine) throws ServerException {
    ...
    try (@SuppressWarnings("unused")
      ...
      if (environmentHolder != null && environmentHolder.status != EnvStatus.STOPPING) {
        ...
        throw new ServerException(
            format(
                "Can't add machine into environment. Environment of workspace '%s' is missing",
  ...
  private void removeMachine(String workspaceId, String machineId) {
    try (@SuppressWarnings("unused")
        Unlocker u = stripedLocks.writeLock(workspaceId)) {
      EnvironmentHolder environmentHolder = environments.get(workspaceId);
      if (environmentHolder != null) {
  ...
  private void destroyEnvironment(String networkId, List<Instance> machines) {
    for (Instance machine : machines) {
      try {
        ...
        LOG.error(
            format(
                "Could not destroy machine '%s' of workspace '%s'",
  ...
  void init() {
    if (!(machineLogsDir.exists() || machineLogsDir.mkdirs())) {
      throw new IllegalStateException(
          "Unable create directory " + machineLogsDir.getAbsolutePath());
    }
  ...
  /**
   * Removes all descriptors from the in-memory storage, while {@link
   * MachineProcessManager#cleanup()} is responsible for machines destroying.
   */
  @PreDestroy
  @VisibleForTesting
  @SuppressWarnings("unused")
  ...
  private LineConsumer getMachineLogger(
      MessageConsumer<MachineLogMessage> environmentLogger, String machineId, String machineName)
      throws ServerException {
    ...
    try {
      return new ConcurrentCompositeLineConsumer(
          new ConcurrentFileLineConsumer(getMachineLogsFile(machineId)), lineConsumer);
    } catch (IOException e) {
      throw new MachineException(
          format(
              "Unable create log file '%s' for machine '%s'.", e.getLocalizedMessage(), machineId));
    }
  }
  ...
  private void ensurePreDestroyIsNotExecuted() throws ServerException {
    if (isPreDestroyInvoked) {
      throw new ServerException(
          "Could not perform operation because application server is stopping");
    }
  }
  ...
  private static class EnvironmentHolder {
    ...
    public int hashCode() {
      return Objects.hash(
          startQueue, machines, status, logger, name, environmentConfig, environment);
    }
  }
      ...
      if ((eventType == OOM) || (eventType == DIE)) {
        sharedPool.execute(
            () -> {
              ...
              MachineStatusEvent destroyedEvent =
                  newDto(MachineStatusEvent.class)
                      .withEventType(MachineStatusEvent.EventType.DESTROYED)
              ...
              try {
                ...
                LOG.warn(
                    "Destroying of machine {} in workspace {} where container was unexpectedly stopped failed. Error: {}, {}",
                    machineId,
                    workspaceId,
