...
public class WorkspaceManager {
   ...
   * @throws NullPointerException when either {@code config} or {@code owner} is null
  ...
  public WorkspaceImpl startWorkspace(
    ...
    final boolean autoRestore =
        restoreAttr == null ? defaultAutoRestore : parseBoolean(restoreAttr);
    startAsync(
        workspace,
        envName,
        firstNonNull(restore, autoRestore) && !getSnapshot(workspaceId).isEmpty());
    runtimes.injectRuntime(workspace);
    addExtraAttributes(workspace);
    return workspace;
  }
  ...
  public WorkspaceImpl startWorkspace(WorkspaceConfig config, String namespace, boolean isTemporary)
    ...
    final WorkspaceImpl workspace =
        doCreateWorkspace(config, accountManager.getByName(namespace), emptyMap(), isTemporary);
  ...
  /**
   * Asynchronously stops the workspace, creates a snapshot of it if {@code createSnapshot} is set
   * to true.
   *
   * @param workspaceId the id of the workspace to stop
   * @param createSnapshot true if create snapshot, false if don't, null if default behaviour should
   *     be used
   * @throws ServerException when any server error occurs
   * @throws NullPointerException when {@code workspaceId} is null
   * @throws NotFoundException when workspace {@code workspaceId} doesn't have runtime
   */
  public void stopWorkspace(String workspaceId, @Nullable Boolean createSnapshot)
      throws ConflictException, NotFoundException, ServerException {
    requireNonNull(workspaceId, "Required non-null workspace id");
    final WorkspaceImpl workspace = workspaceDao.get(workspaceId);
    workspace.setStatus(runtimes.getStatus(workspaceId));
    if (workspace.getStatus() != WorkspaceStatus.RUNNING
        && workspace.getStatus() != WorkspaceStatus.STARTING) {
      throw new ConflictException(
          format(
   ...
   *
   * <p>Basically creates {@link SnapshotImpl snapshot} instance for each machine from runtime
   * workspace's active environment.
   *
   * <p>If snapshot of workspace's dev machine was created successfully publishes {@link
   * EventType#SNAPSHOT_CREATED} event, otherwise publishes {@link
   * EventType#SNAPSHOT_CREATION_ERROR} with appropriate error message.
   *
   * <p>Note that: <br>
   * Snapshots are created asynchronously <br>
   * If snapshot creation for one machine failed, it wouldn't affect another snapshot creations
   *
   * @param workspaceId runtime workspace id
   * @throws NullPointerException when {@code workspaceId} is null
   * @throws NotFoundException when runtime workspace with given id does not exist
   * @throws ServerException when any other error occurs
   * @throws ConflictException when workspace is not running
   */
   ...
   *
   * @param workspaceId workspace id to get snapshot
   ...
   * @throws ServerException when any other error occurs
   */
  public List<SnapshotImpl> getSnapshot(String workspaceId)
   ...
   * @throws NotFoundException if machine is not found in running workspace
   * @throws ConflictException if workspace is not running
   * @throws ConflictException if machine stop is forbidden (e.g. machine is dev-machine)
   ...
   * @throws NotFoundException if workspace is not running
   * @throws NotFoundException if machine is not found in the workspace
   */
  public Instance getMachineInstance(String workspaceId, String machineId)
      throws NotFoundException, ServerException {
    requireNonNull(workspaceId, "Required non-null workspace id");
    requireNonNull(machineId, "Required non-null machine id");
    workspaceDao.get(workspaceId);
  ...
  /**
   * Shuts down workspace service and waits for it to finish, so currently starting and running
   * workspaces are stopped and it becomes unavailable to start new workspaces.
   *
   * @throws InterruptedException if it's interrupted while waiting for running workspaces to stop
  ...
  /**
   * Stops all the running and starting workspaces - snapshotting them before if needed. Workspace
   * stop operations executed asynchronously while the method waits for async task to finish.
   */
      ...
      for (WorkspaceImpl workspace : runningOrStarting) {
        try {
          ...
          if (runtimes.hasRuntime(workspace.getId())) {
            LOG.warn(
                "Couldn't stop the workspace '{}' normally, due to error: {}",
                workspace.getId(),
                x.getMessage());
          }
        }
  ...
  private CompletableFuture<Void> stopAsync(
      WorkspaceImpl workspace, @Nullable Boolean createSnapshot)
      throws ConflictException, NotFoundException, ServerException {
    if (!workspace.isTemporary()) {
      workspace.getAttributes().put(UPDATED_ATTRIBUTE_NAME, Long.toString(currentTimeMillis()));
      workspaceDao.update(workspace);
    ...
    return sharedPool.runAsync(
        () -> {
          final String stoppedBy =
              sessionUserNameOr(workspace.getAttributes().get(WORKSPACE_STOPPED_BY));
          LOG.info(
          ...
          if (snapshotBeforeStop) {
            try {
              ...
              LOG.warn(
                  "Could not create a snapshot of the workspace '{}/{}' "
    ...
    sharedPool.execute(
        () -> {
          try {
            ...
            LOG.warn(
                "Error occurs on start of additional machine in workspace %s. Error: %s",
  ...
  private void checkWorkspaceIsRunning(WorkspaceImpl workspace, String operation)
  ...
  private void addExtraAttributes(WorkspaceImpl workspace) throws SnapshotException {
    ...
    if (!snapshots.isEmpty()) {
      workspace
          .getAttributes()
          .put(SNAPSHOTTED_AT_ATTRIBUTE_NAME, Long.toString(snapshots.get(0).getCreationDate()));
    }
  }
}
